...
use Parameter\models\ParameterModel; // EDISYUM - LBE01 Correctif recherche contacts
...
class AutoCompleteController
{
    ...
    public static function getCorrespondents(Request $request, Response $response): Response
    {
        ...
        $indexDeleted = false;
        foreach ($autocompleteData as $key => $value) {
            $autocompleteData[$key] = ContactController::getContactConfidentiality($value, true);
            if (empty($autocompleteData[$key])) {
                $indexDeleted = true;
                unset($autocompleteData[$key]);
            }
        }
        if ($indexDeleted) {
            $autocompleteData = array_values($autocompleteData);
        }
        // END EDISSYUM - NCH01
    ...
    public static function getUsersForCircuit(Request $request, Response $response): Response
    {
        ...
        $users = DatabaseModel::select([
            'select'   => ['DISTINCT users.id', 'users.firstname', 'users.lastname', 'users.external_id'],
    ...
    public static function getBanAddresses(Request $request, Response $response): Response
    {
        ...
        // EDISSYUM - NCH01 Augmentation du nombre de résultats pour la BAN
        $maxItems = ParameterModel::getById(['id' => 'banMaxItems']);
        if (count($maxItems) > 0) {
            $limit = $maxItems['param_value_int'];
        } else {
            $limit = self::TINY_LIMIT;
        ...
        foreach ($hits as $hit) {
            if (count($addresses) >= $limit) { // EDISSYUM - NCH01 Augmentation du nombre de résultats pour la BAN | Remplacer self::TINY_LIMIT par $limit
                break;
            }
        ...
        Zend_Search_Lucene::setResultSetLimit($limit); // EDISSYUM - NCH01 Augmentation du nombre de résultats pour la BAN | Remplacer 10 par $limit
