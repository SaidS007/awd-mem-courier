...
class EmailController
{
    ...
    public static function createEmail(array $args): array|int
    {
        ...
        if ($args['data']['status'] != 'DRAFT') {
            // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
            if(
                array_key_exists('mailSecChecked', $args['data'])
                && $args['data']['mailSecChecked']
            ){
                $args['emailId'] = $id;
                $isSent = EmailController::createMailSec($args);
                ...
                if ($args['data']['status'] == 'EXPRESS') {
                    ...
                    $command .= "{$customId} {$id} {$args['userId']} \"{$encryptKey}\" '{$options}' '{$args['data']['receivedReceipt']}' '{$args['data']['pjLinkChecked']}' > /dev/null &"; // EDISSYUM - NCH01 Ajout d'un accusé de lecture | ajout receivedReceipt. AMO01 - Rajout d'une option pour envoyer les PJ via liens épémères. OBR01 - Fix si l'encypt key contient des '
                    exec($command);
                }
            } // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        $args['data'] = $body; // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
        ...
        if ($body['status'] != 'DRAFT') {
            ...
            // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
            if ($body['mailSecChecked']) {
                $isSent = EmailController::createMailSec($args);
                if (!empty($isSent['success'])) {
                    EmailModel::update(['set' => ['status' => 'SENT', 'send_date' => 'CURRENT_TIMESTAMP'], 'where' => ['id = ?'], 'data' => [$args['id']]]);
                } else {
                    EmailModel::update(['set' => ['status' => 'ERROR', 'send_date' => 'CURRENT_TIMESTAMP'], 'where' => ['id = ?'], 'data' => [$args['id']]]);
                }
            } else { // END - EDISSYUM ASY01
                $encryptKey = CoreConfigModel::getEncryptKey();
                $command = "php src/app/email/scripts/sendEmail.php ";
    ...
    public static function sendEmail(array $args): array
    {
        ...
        if (in_array($configuration['type'], ['smtp', 'mail'])) {
            ...
            // EDISSYUM - NCH01 Ajout d'une possibilité de connexion avec du TLS non sécurisé
            if ($configuration['secure'] == 'tls_insecure') {
        ...
        if ($args['receivedReceipt']) {
            $phpmailer->addCustomHeader("Disposition-Notification-To: " . $email['sender']['replyTo']);
        }
        // END EDISSYUM - NCH01
        ...
        if ($email['message_exchange_id']) {
            ...
            if (!empty($email['document']['attachments'])) {
                $my_i = 0; // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères
                foreach ($email['document']['attachments'] as $attachment) {
                    ...
                    // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères - Pièces jointes - Attachements attachments
                    ...
                    $my_i += 1; // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères
                }
            }
            if (!empty($email['document']['notes'])) {
                $encodedDocument = NoteController::getEncodedPdfByIds(['ids' => $email['document']['notes']]);

                // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères - Notes - Annotations
                if ($CheckboxIsCheckedAndNextcloudArgsDefined) {
    ...
    // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
    public static function createMailSec(array $args)
    {
        ValidatorModel::notEmpty($args, ['data']);
        ValidatorModel::arrayType($args, ['data', 'options']);

        $pastellStatus = false;
        $res = ParameterController::checkPastellConnection();
        if ($res['result'] == 'OK') {
            $pastellStatus = true;
        }

        if ($pastellStatus === true) {
            $mailSecConfiguration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_send_secure_email', 'select' => ['value']]);
            $mailSecConfiguration = !empty($mailSecConfiguration['value']) ? json_decode($mailSecConfiguration['value'], true) : [];

            $pastell_site = rtrim($mailSecConfiguration['pastell']['url'], '/') . '/api/v2';
            $login = $mailSecConfiguration['pastell']['login'];
            $password = $mailSecConfiguration['pastell']['password'];
            $entite = $mailSecConfiguration['pastell']['entite'];

            ////////// Creation du mail securisé ///////////////
            $curlResponse = CurlModel::exec([
                'url' => $pastell_site . '/entite/' . $entite . '/document',
                'basicAuth' => ['user' => $login, 'password' => $password],
                'headers' => ['content-type:application/json'],
                'method' => 'POST',
                'queryParams' => ['type' => 'mailsec', 'action' => 'envoi'],
                'body' => json_encode([])
            ]);
            /////////////////////////////////////////////////

            if (!empty($curlResponse['response']['id_d'])) {
                ////////// Modification de mail securisé ///////////////
                $bodyData = array(
                    'libelle' => $args['data']['object'],
                    'type' => 'mailsec',
                    'objet' => $args['data']['object'],
                    'to' => implode(',', $args['data']['recipients']),
                    'message' => $args['data']['body'],
                    'cc' => empty($args['data']['cc']) ? '' : implode(',', $args['data']['cc']),
                    'bcc' => empty($args['data']['cci']) ? '' : implode(',', $args['data']['cci'])
                );

                $curlResponseUpdate2 = CurlModel::exec([
                    'url' => $pastell_site . '/entite/' . $entite . '/document/' . $curlResponse['response']['id_d'],
                    'basicAuth' => ['user' => $login, 'password' => $password],
                ...
                if (!empty($args['data']['document'])) {
                    $fileNames = array(); //tableau contenant les noms de fichiers
                    $filePaths = array(); //tableau contenant le lien de fichiers

                    if ($args['data']['document']['isLinked']) {
                        $encodedDocument = ResController::getEncodedDocument(['resId' => $args['data']['document']['id'], 'original' => $args['data']['document']['original']]);
                        $tmpPath = CoreConfigModel::getTmpPath();
                        $filePath = $tmpPath . 'document_' . $args['data']['document']['id'] . '.pdf';
                        $filename = 'Document_' . $args['data']['document']['id'] . '.pdf';
                        file_put_contents($filePath, base64_decode($encodedDocument['encodedDocument']));
                    ...
                    if (!empty($args['data']['document']['attachments'])) {
                        ...
                        foreach ($args['data']['document']['attachments'] as $attachment) {
                            ...
                            if (sizeof($args['data']['document']['attachments']) > 0) {
                                $resource_attach = DatabaseModel::select([
                                    'select' => ['docserver_id', 'path', 'filename', 'title', 'format'],
                                    'table' => ['res_attachments'],
                                    'where' => ['res_id = ?'],
                                    'data' => [$args['data']['document']['attachments'][$my_i]['id']],
                                ]);
                                $filePath = $tmpPath . 'attachements_' . $args['data']['document']['attachments'][$my_i]['id'] . '.' . $resource_attach[0]['format'];
                                $filename = str_replace(" ", "_", $resource_attach[0]['title']) . '.' . $resource_attach[0]['format'];
                                file_put_contents($filePath, base64_decode($encodedDocument['encodedDocument']));
                            }
                            $my_i += 1;
                    ...
                    if (!empty($args['data']['document']['notes'])) {
                        $encodedDocument = NoteController::getEncodedPdfByIds(['ids' => $args['data']['document']['notes']]);
                        $filePath = $tmpPath . 'notes_' . $args['data']['document']['id'] . '.pdf';
                        $filename = 'Notes_' . $args['data']['document']['id'] . '.pdf';
                        file_put_contents($filePath, base64_decode($encodedDocument['encodedDocument']));
                    ...
                    ////////////////////// On parcours le tableau de documents et on join au document ////////////////
                    if (count($fileNames) > 0) {
                        for ($i = 0, $size = count($fileNames); $i < $size; ++$i) {

                            $bodyData = array(
                                'file_name' => $fileNames[$i],
                                'file_content' => file_get_contents($filePaths[$i])
                            );

                            $curlResponsePJ = CurlModel::exec([
                                'url' => $pastell_site . '/entite/' . $entite . '/document/' . $curlResponse['response']['id_d'] . '/file/document_attache/' . $i,
                                'basicAuth' => ['user' => $login, 'password' => $password],
                                'headers' => ['Content-Type' => 'application/x-www-form-urlencoded'],
                                'method' => 'POST',
                                'body' => http_build_query($bodyData)
                ...
                /////////////// Envoie de mail securisé  au destinateur ///////////////
                $curlResponseSend = CurlModel::exec([
                    'url' => $pastell_site . '/entite/' . $entite . '/document/' . $curlResponse['response']['id_d'] . '/action/envoi',
                    'basicAuth' => ['user' => $login, 'password' => $password],
                    'headers' => ['Content-Type' => 'application/x-www-form-urlencoded'],
                    'method' => 'POST',
                ]);
                ...
                ////////////////////// Recuperation du status du document //////////////////////
                $curlResponseGet = CurlModel::exec([
                    'url' => $pastell_site . '/entite/' . $entite . '/document/' . $curlResponse['response']['id_d'],
                    'basicAuth' => ['user' => $login, 'password' => $password],
                    'method' => 'GET'
                ])['response'];
                ...
                /////////// Mise à jour du statut du mail dans la table emails //////////////////
                if ($curlResponseGet['last_action']['action'] == "envoi") {
                    EmailModel::update(['set' => ['status' => 'SENT', 'send_date' => 'CURRENT_TIMESTAMP'], 'where' => ['id = ?'], 'data' => [$args['emailId']]]);
                } else {
                    EmailModel::update(['set' => ['status' => 'ERROR', 'send_date' => 'CURRENT_TIMESTAMP'], 'where' => ['id = ?'], 'data' => [$args['emailId']]]);
                }
            }
        } else {
            return ['errors' => 'Connexion Pastell failed'];
        }
