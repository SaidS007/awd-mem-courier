...
abstract class NotificationScheduleModelAbstract
{
    ...
    public static function saveCrontab(array $aArgs = []): bool
    {
        ...
        // EDISSYUM - NCH01 Fix de la création des crontabs depuis l'interface
        file_put_contents(CoreConfigModel::getTmpPath() . '/crontab.plain', print_r($file, true));
        file_put_contents(CoreConfigModel::getTmpPath() . '/crontab.txt', $output);
        exec('crontab ' . CoreConfigModel::getTmpPath() . '/crontab.txt');

        unlink(CoreConfigModel::getTmpPath() . '/crontab.plain');
        unlink(CoreConfigModel::getTmpPath() . '/crontab.txt');
        // END EDISSYUM - NCH01

        // EDISSYUM - NCH01 Fix de la création des crontabs depuis l'interface | Commenter le bloc ci-dessous
