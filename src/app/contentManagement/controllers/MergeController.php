...
class MergeController
{
    ...
    private static function getDataForMerge(array $args): array
    {
        ...
        $settings = ParameterModel::getById(['id' => 'customPostalAddressSettings', 'select' => ['param_value_string']]);
        if ($settings) {
            $settings = json_decode($settings['param_value_string'], true);
            $dataToBeMerge['recipient']['customPostalAddress'] = self::retrieveCustomPostalAddress($recipient, $settings);
            $dataToBeMerge['sender']['customPostalAddress'] = self::retrieveCustomPostalAddress($sender, $settings);
            if (empty($args['inMailing'])) {
                $dataToBeMerge['attachmentRecipient']['customPostalAddress'] = self::retrieveCustomPostalAddress($dataToBeMerge['attachmentRecipient'], $settings);
            }
        }
        // END EDISSYUM - NCH01
    ...
    private static function retrieveDestinationByEntityType($childDestination, $entityType) {
        if (!empty($childDestination['parent_entity_id'])) {
...
//        $generator->writeBarcodeFile($barcodeFile);
        ...
        if ($barcodeFile && $args['chrono']) {
            ...
            $qrCodeResult->saveToFile($barcodeFile);
        }
        // END EDISSYUM - NCH01
    ...
    // EDISSYUM - NCH01 Rajout d'une variable de fusion custom pour les adresses - customPostalAddress
    private static function retrieveCustomPostalAddress($data, $settings): string {
        $lines = '';
        if ($settings) {
            foreach ($settings as $line) {
                $lineValues = explode(';', $line['value']);
                $lineContent = '';

                foreach ($lineValues as $value) {
                    $lineContent .= $data[trim($value)] . ' ';
                }

                $lineContent = substr(trim($lineContent), 0, $line['count']);
                if ($line['case'] == 'upper') {
                    $lineContent = mb_convert_case($lineContent, MB_CASE_UPPER, 'UTF-8');
                } elseif ($line['case'] == 'lower') {
                    $lineContent = mb_convert_case($lineContent, MB_CASE_LOWER, 'UTF-8');
                } elseif ($line['case'] == 'ucfirst') {
                    $lineContent = mb_strtoupper(mb_substr($lineContent, 0, 1)) . mb_substr($lineContent, 1);
                } elseif ($line['case'] == 'ucwords') {
                    $lineContent = mb_convert_case($lineContent, MB_CASE_TITLE, 'UTF-8');
                }
