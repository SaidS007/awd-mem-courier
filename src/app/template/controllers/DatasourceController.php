    ...
    public static function letterboxEvents(array $aArgs): array
    {
        ...
        foreach ($aArgs['params']['events'] as $event) {
            ...
            if (!empty($res['destination'])) {
                $destination = EntityModel::getByEntityId(['entityId' => $res['destination'], 'select' => ['*']]);
                $destination['path'] = EntityModel::getEntityPathByEntityId(['entityId' => $res['destination'], 'path' => '']);
                $datasources['destination'][] = $destination;
            }
