...
use Configuration\models\ConfigurationModel; // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères
...
class ParameterController
{
        ...
        if (
            !in_array(
                ...
                [
                    ...
                    'contactsConfidentiality', // EDISSYUM - NCH01 Rajout de la confidentialité des contacts | rajout de contactsConfidentiality
                    'summarySheetMandatory', // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | rajout de summarySheetMandatory
                    'printedFolderName', // EDISSYUM - NCH01 Création d'un masque pour le nom de fichier du dossier d'impression | rajout de printedFolderName
                    'sendSummarySheet', // EDISSYUM - (AMI01 + AMO01) Intégration fiche de liaison au parapheur externe | rajout de sendSummarySheet
                    'defaultStatusSummarySheet', // EDISSYUM - (AMI01 + AMO01) Intégration fiche de liaison au parapheur externe | rajout de defaultStatusSummarySheet
                    'defaultParapheurSummarySheet', // EDISSYUM - (AMI01 + AMO01) Intégration fiche de liaison au parapheur externe | rajout de defaultParapheurSummarySheet,
                    'showFoldersByDefault' // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers | rajout de showFoldersByDefault
                ]
            )
    ...
    public function create(Request $request, Response $response)
    {
        ...
        // ParameterModel::create($data); // EDISSYUM - NCH01 Fix erreur lors de la création/màj de paramètre chrono_xml | Commenter la ligne
        if (strpos($data['id'], 'chrono_') !== false) {
            if (!Validator::notEmpty()->intVal()->validate($data['param_value_int'])) {  // EDISSYUM - NCH01 Fix erreur lors de la création/màj de paramètre chrono_xml | rajout de ! devant Validator
        ...
        ParameterModel::create($data); // EDISSYUM - NCH01 Fix erreur lors de la création/màj de paramètre chrono_xml
    ...
    public static function checkPastell(Request $request, Response $response)
    {
        $res = self::checkPastellConnection();
        if (empty($res['error'])) {
            return $response->withStatus(200)->withJson(['result' => 'OK']);
        }
    ...
    public static function checkPastellConnection() {
        $configuration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_send_secure_email', 'select' => ['value']]);
        $configuration = !empty($configuration['value']) ? json_decode($configuration['value'], true) : [];

        $pastell_site = rtrim($configuration['pastell']['url'], '/') . '/api/v2';
        $login = $configuration['pastell']['login'];
        $password = $configuration['pastell']['password'];
        $entite = $configuration['pastell']['entite'];

        $response = CurlModel::exec([
            'url' => $pastell_site . '/entite',
        ...
        foreach ($response["response"] as $res_entite) {
            if (strcmp($res_entite['id_e'], $entite) == 0) {
                $result = true;
                break;
            }
        }
