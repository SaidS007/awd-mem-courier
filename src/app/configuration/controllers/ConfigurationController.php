...
class ConfigurationController
{
    ...
    public function getByPrivilege(Request $request, Response $response, array $args): Response
    {
        ...
        // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères | Commenter ce bloc
//        elseif (
...
//        }
        ...
        if ($args['privilege'] != 'admin_attachments_hosts' && $args['privilege'] != 'admin_send_secure_email' && !PrivilegeController::hasPrivilege(['privilegeId' => $args['privilege'], 'userId' => $GLOBALS['id']])) { // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères - Ajout variable admin_attachments_hosts // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
            return $response->withStatus(403)->withJson(['errors' => 'Service forbidden']);
        } elseif (!PrivilegeController::hasPrivilege(['privilegeId' => $args['privilege'], 'userId' => $GLOBALS['id']]) && $args['privilege'] != 'admin_attachments_hosts' && $args['privilege'] != 'admin_send_secure_email') { // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères - Ajout variable admin_attachments_hosts // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
            return $response->withStatus(403)->withJson(['errors' => 'Service forbidden']);
        }
        ...
        if (
            !in_array(
                ...
                [
                    ...
                    'admin_attachments_hosts', // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères - Ajout variable admin_attachments_hosts
                    'admin_send_secure_email', // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell - Ajout variable admin_send_secure_email
                    'admin_search_contacts', // EDISSYUM - NCH01 Fenetre de recherche de contacts Ajout admin_search_contacts
                    'admin_search_folders' // EDISSYUM - EME01  Ajout d'une fenêtre pour administrer la recherche des dossiers | Ajouter  'admin_search_folders'
                ]
            )
        ) {
