...
{
    private const ALTERNATIVES_CONNECTIONS_METHODS = ['sso', 'cas', 'ldap', 'keycloak', 'shibboleth', 'azure_saml', 'oidc']; // EDISSYUM - NCH01 Ajout d'une méthode de connexion OIDC
    ...
    public function get(Request $request, Response $response): Response
    {
        ...
        if (UserController::isRoot(['id' => $GLOBALS['id']])) {
            ...
            $usersNoEntities = UserEntityModel::getUsersWithoutEntities(
                ['select' => ['id', 'users.user_id', 'firstname', 'lastname', 'status', 'mail', 'mode', 'external_id']]
            );
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        $user = UserModel::getById(
            ['id' => $args['id'], 'select' => [
                'id', 'firstname', 'lastname', 'status', 'mail', 'phone',
                'external_id', 'preferences' // EDISSYUM - NCH01 Amélioration des webservices utilisateurs | Ajout external_id et preferences
            ]]
        );
    ...
    public function update(Request $request, Response $response, array $aArgs): Response
    {
        ...
        $user = UserModel::getById(['id' => $aArgs['id'], 'select' => [
            'status', 'mode',
            'external_id', 'preferences' // EDISSYUM - NCH01 Amélioration des webservices utilisateurs | Ajout d'external_id et preferences
        ]]);
    ...
    public function suspend(Request $request, Response $response, array $args): Response
    {
        ...
        $user = UserModel::getById([
            'id' => $args['id'],
            'select' => ['firstname', 'lastname', 'user_id', 'mode', 'status'] // EDISSYUM - NCH01 Fix pour éviter le passage en SPD des utilisateurs INACT lors de la synchro LDAP | rajout de status
        ]);
        if (
            in_array($user['mode'], ['root_visible', 'root_invisible']) &&
            !UserController::isRoot(['id' => $GLOBALS['id']])
    ...
    public function setRedirectedBaskets(Request $request, Response $response, array $args): Response
    {
        ...
        if ($env->isNewInternalParapheurEnabled()) {
            ...
            foreach ($body as $redirectedBasket) {
                $ownerUser = (empty($redirectedBasket['originalOwner'])) ?
                    $userRepository->getUserById($args['id']) :
                    $userRepository->getUserById($redirectedBasket['originalOwner']);
                ...
                if (!in_array([$ownerUser->getId(), $destUser->getId()], $addedSubstitutes)) {
                    try {
                        $addSubstitute->addSubstitute($ownerUser, $destUser);
    ...
    public function getExport(Request $request, Response $response): Response
    {
        ...
        // EDISSYUM - NCH01 Fix pour éviter les doublons dans l'export des utilisateurs
        $userIdPresent = array_search('users.id', $select);
        if (!$userIdPresent) {
            $select[] = 'users.id';
        }
        // END EDISSYUM - NCH01
        ...
        $mapDecodeFunc = function(string $utf8_string): string {
            return mb_convert_encoding($utf8_string, 'ISO-8859-1', 'UTF-8');
        };

        fputcsv($file, array_map($mapDecodeFunc, array_values($csvHead)), $delimiter); // EDISSYUM - NCH01 Amélioration de l'encodage lors de l'export des utilisateurs | rajout du array_map
        ...
        $usersIdList = []; // EDISSYUM - NCH01 Fix pour éviter les doublons dans l'export des utilisateurs
        foreach ($users as $user) {
            $csvContent = [];
            // EDISSYUM - NCH01 Fix pour éviter les doublons dans l'export des utilisateurs
            if (in_array($user['id'], $usersIdList)) {
                continue;
            }
            // END EDISSYUM - NCH01
            foreach ($fields as $field) {
                ...
                $usersIdList[] = $user['id']; // EDISSYUM - NCH01 Fix pour éviter les doublons dans l'export des utilisateurs
            }
            $mapDecodeFunc = function(string $utf8_string): string {
                return mb_convert_encoding($utf8_string, 'ISO-8859-1', 'UTF-8');
            };

            fputcsv($file, array_map($mapDecodeFunc, array_values($csvContent)), $delimiter); // EDISSYUM - NCH01 Amélioration de l'encodage lors de l'export des utilisateurs | rajout du array_map
        }
