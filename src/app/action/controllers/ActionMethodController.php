...
use Slim\Psr7\Request; // EDISSYUM - NCH01 Rajout de l'export PESv2 dans la recherche
use SrcCore\http\Response; // EDISSYUM - NCH01 Rajout de l'export PESv2 dans la recherche
...
class ActionMethodController
{
    ...
    public static function redirect(array $args): bool|array
    {
        ...
        $controller = ListInstanceController::updateListInstance([
            ...
            'userId'    => $GLOBALS['id'],
            'actionId'  => $args['action']['id'], // EDISSYUM - NCH01 Amélioration de l'historique de la listinstance pour l'IA
            'fullRight' => true
        ]);
    ...
    public static function rejectVisaBackToPrevious(array $args): bool|array
    {
        ...
        if ($env->isNewInternalParapheurEnabled()) {
            ...
            try {
                $rejectVisaBackToPreviousAction->execute($args['resId']);
            } catch (Throwable $th) {
    ...
    public static function interruptVisa(array $args): bool|array
    {
        ...
        try {
            $interruptCircuitAction->execute($args['resId'], $args['note']);
        } catch (Throwable $th) {
            return ['errors' => [$th->getMessage()]];
        }
...
//        }
    ...
    public static function exportPESV2(array $args) {
        ...
        $loadedXml = CoreConfigModel::getXmlLoaded(['path' => 'config/file_export.xml']);
        if ($loadedXml) {
            ...
            for ($i = 0; $i < count($args['resources']); $i++) {
                ...
                if ($resource) {
                    ...
                    $name = self::generate_name($args['resources'][$i], $view, $globality, NULL, '', NULL );
                    $finalName = $path . $name;
                    ...
                    if (copy($tmpFilePath, $finalName)) {
                        ...
                        if (!empty($listAttachments)) {
                            ...
                            foreach ($listAttachments as $attachment) {
                                ...
                                if ($resourceAttachment) {
                                    ...
                                    $attachmentName = self::generate_name($args['resources'][$i], $view, $globality, $attachmentSuffix, $attachmentNumber, $attachmentExtension);
                                    ...
                                    if (copy($tmpAttachmentFilePath, $attachmentFinalName)) {
                                        ...
                                        if ($exportToZip) {
                                            $listOfFiles[] = $attachmentFinalName;
                                        }
                                    } else {
                        ...
                        if (count($args['resources']) == 1) {
                            return ['errors' => ['[exportPESV2] [resId ' . $args['resources'][$i] . '] ' . _COPY_ERROR]];
                        }
                    }
                } else {
                    if (count($args['resources']) == 1) {
                        return ['errors' => ['[exportPESV2] [resId ' . $args['resources'][$i] . '] ' . _ERROR_WHILE_RETRIEVING_FILE]];
                    }
                }
            }
            if ($exportToZip == 'true') {
    ...
    private static function generate_name($resId, $view, $structure, $suffixAttachment = NULL, $attachmentNumber = '', $attachmentExtension = NULL) {
        $tmp = $structure;
