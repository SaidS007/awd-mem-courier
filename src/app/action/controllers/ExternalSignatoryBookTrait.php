...
use ExternalSignatoryBook\controllers\LexPersonaController; // EDISSYUM - YAA01 Ajout du connecteur LexPersona
use ExternalSignatoryBook\controllers\SesileController; // EDISSYUM - YAA01 Ajout du connecteur Sesile
    ...
    public static function sendExternalSignatoryBookAction(array $args): array
    {
        ...
        if (!empty($loadedXml)) {
            ...
            if ($config['id'] == 'maarchParapheur') {
                ...
                // EDISSYUM - PYB01 Ajout du connecteur Pastell | Commentez le code ci-dessous
...
//                $sentInfo = $sendToPastell->sendData($args['resId']);
            ...
            elseif ($config['id'] == 'lexPersona') { // EDISSYUM - YAA01 Ajout du connecteur lexPersona
                $sentInfo = LexPersonaController::sendDatas([
                    'config' => $config,
                    'resIdMaster' => $args['resId'],
                    'actionId' => $args['data']['actionId'],
                    'data' => $args['data']['lexPersona'],
                    'note' => $args['note']['content'] ?? null
                ]);
            } // END EDISSYUM - YAA01
            // EDISSYUM - YAA01 Ajout du connecteur Sesile
            elseif ($config['id'] == 'sesile') {
                $sentInfo = SesileController::sendDatas([
                    'config' => $config,
                    'resIdMaster' => $args['resId'],
                    'selectedFormData' => $args['data']['selectedFormData'],
                    'note' => $args['note']['content'] ?? null
                ]);
            } // END EDISSYUM - YAA01
            ...
            if (!empty($args['action']['parameters']['visaValidation'])) {
                $listInstances = \Entity\models\ListInstanceModel::get([
                    'select'    => ['listinstance_id'],
                    'where'     => ['res_id = ?', 'item_type = ?', 'process_date IS NULL', 'item_mode = ?'],
                    'data'      => [$args['resId'], 'user_id', 'visa'],
