...
class PreProcessActionController
{
    ...
    public function checkAcknowledgementReceipt(Request $request, Response $response, array $args): Response
    {
        ...
        foreach ($data['resources'] as $resId) {
            $resource = ResModel::getById([
                'select' => ['res_id', 'category_id', 'alt_identifier', 'type_id', 'destination', 'model_id'], // EDISYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement
                'resId' => $resId
            ]);
            ...
            // EDISSYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement
            $generateAr = false;
            $model = IndexingModelModel::getById(['id' => $resource['model_id'], 'select' => ['generate_ar']]);
            if (!empty($model) && $model['generate_ar'] === true) {
                $generateAr = true;
            }
            // END EDISSYUM - NCH01

            if ($resource['category_id'] != 'incoming' && !$generateAr) { // EDISSYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement | Ajout de $generateAr
                $noSendAR['number'] += 1;
                $noSendAR['list'][] = [
                    'resId'          => $resId,
                    'alt_identifier' => $resource['alt_identifier'],
            ...
            if ($email > 0) {
                ...
                if ((!empty($emailSender) && is_array($emailSender)) || $currentMode == 'manual') {
                    ...
                    $sendEmail += $email;
                } elseif ($emailSender == null && $currentMode != 'manual') {
                    $noSendAR['number'] += 1;
                    $noSendAR['list'][] = [
                        'resId'          => $resId,
                        'alt_identifier' => $resource['alt_identifier'],
    ...
    public function checkExternalSignatoryBook(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if (!empty($loadedXml)) {
            ...
            if (
                in_array($signatureBookEnabled, ['maarchParapheur', 'fastParapheur', 'iParapheur', 'ixbus', 'pastell', 'blueway', 'lexPersona', 'sesile']) // EDISSYUM - PYB01 Ajout du connecteur Blueway, Ajout du connecteur Pastell| added blueway, pastell // EDISSYUM - YAA01 Ajout du connecteur lexPersona, Ajout du connecteur sesile | added lexPersona, sesile
            ) {
                ...
                foreach ($data['resources'] as $resId) {
                    ...
                    $integratedResource = ResModel::get([
                        'select' => ['subject', 'alt_identifier', 'path', 'filename', 'docserver_id'],
                        'where'  => [
                    ...
                    if (
                        empty($integratedResource) ||
                        empty($integratedResource[0]['path']) ||
                        empty($integratedResource[0]['filename']) ||
                        empty($integratedResource[0]['docserver_id'])
                    ) {
    ...
    public function checkContinueVisaCircuit(Request $request, Response $response, array $args): Response
    {
        ...
        if ($env->isNewInternalParapheurEnabled()) {
            ...
            $visaWorkflowRepository = new VisaWorkflowRepository(new UserFactory());
        ...
        foreach ($body['resources'] as $resId) {
            ...
            if (empty($isSignatory[0])) {
                ...
                if ($env->isNewInternalParapheurEnabled()) {
                    $warningInfo['hasDigitalSignature'] = $mainResource->getHasDigitalSignature();

                    if (!$warningInfo['hasDigitalSignature']) {
                        $attachments = $attachmentRepository->getAttachmentsInSignatureBookByMainResource(
                            $mainResource
                        );
                        foreach ($attachments as $attachment) {
                            if ($attachment->getHasDigitalSignature() === true) {
                                $warningInfo['hasDigitalSignature'] = true;
                            }
                        }
                    }
                }
                ...
                if ($env->isNewInternalParapheurEnabled()) {
                    $successInfo['hasDigitalSignature'] = $mainResource->getHasDigitalSignature();

                    if (!$successInfo['hasDigitalSignature']) {
                        foreach ($attachments as $attachmentInfos) {
                            $attachment = $attachmentRepository->getAttachmentByResId($attachmentInfos['res_id']);
                            if ($attachment->getHasDigitalSignature() === true) {
                                $successInfo['hasDigitalSignature'] = true;
                            }
                        }
                    }
                }
