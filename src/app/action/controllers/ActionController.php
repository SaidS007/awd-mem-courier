...
class ActionController
{
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        if (!empty($body['parameters'])) {
            ...
            if (empty($parameters['keepDestForRedirection'])) { // EDISSYUM - NCH01 Fix création d'action de redirection | Remplacer !empty par empty
                $parameters['keepDestForRedirection'] = false;
            }
            if (empty($parameters['keepCopyForRedirection'])) { // EDISSYUM - NCH01 Fix création d'action de redirection | Remplacer !empty par empty
                $parameters['keepCopyForRedirection'] = false;
            }
            if (empty($parameters['keepOtherRoleForRedirection'])) { // EDISSYUM - NCH01 Fix création d'action de redirection | Remplacer !empty par empty
