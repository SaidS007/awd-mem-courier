...
class VersionUpdateController
{
    ...
    public static function getAvailableFolders(string $migrationFolderPath = 'migration'): array
    {
        ...
        // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion
        foreach ($folderTags as $index => $folder) {
            $specificVersions = ['2301', '2505'];
            if (in_array($folder, $specificVersions)) {
                unset($folderTags[$index]);
                $folderTagsSpecific = array_diff(scandir("$migrationFolderPath/$folder"), array('..', '.', '.gitkeep'));
                natcasesort($folderTagsSpecific);
                foreach ($folderTagsSpecific as $folderTagSpecific) {
                    $folderTags[] = $folderTagSpecific;
                }
            }
        }
        // END EDISSYUM - NCH01
        ...
        foreach ($folderTags as $folder) {
            ...
            if (
                ...
                // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion
                $specificVersions = ['2301', '2505'];
                $folderToScan = "$migrationFolderPath/$folder";
                if (in_array($folderMajorVersion, $specificVersions)) {
                    $folderToScan = "$migrationFolderPath/$folderMajorVersion/$folder";
                }
                // END EDISSYUM - NCH01

                if (is_dir("$folderToScan")) { // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion | Remplacer $migrationFolderPath/$folder par $folderToScan
                    if (!is_readable("$folderToScan")) { // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion | Remplacer $migrationFolderPath/$folder par $folderToScan
                        return ['errors' => "Folder '$folderToScan' is not readable"]; // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion | Remplacer $migrationFolderPath/$folder par $folderToScan
                    }
                    if (count(array_diff(scandir("$folderToScan"), ['.', '..'])) == 0) { // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion | Remplacer $migrationFolderPath/$folder par $folderToScan
                        return ['errors' => "Folder '$folderToScan' is empty, no updates are found!"]; // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion | Remplacer $migrationFolderPath/$folder par $folderToScan
                    }
                    $availableFolders[] = "$folderToScan"; // EDISSYUM - NCH01 Adaptation de la fonction updateSQLVersion | Remplacer $migrationFolderPath/$folder par $folderToScan
                }
            }
        }
    ...
    public static function executeTagSqlFile(string $sqlFilePath, string $docserverMigrationFolderPath): bool
    {
        ...
        if (file_exists($sqlFilePath)) {
            ...
            $backupFile = $docserverMigrationFolderPath . "/backupDB_memcourrier_$actualTime.sql";
