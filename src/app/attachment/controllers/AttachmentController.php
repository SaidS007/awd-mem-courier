...
//        ]);
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        $queryParams = $request->getQueryParams(); // EDISSYUM - NCH01 Fix typist des nouvelles version de PJ

        if (empty($attachment) || in_array($attachment['status'], ['DEL', 'OBS'])) {
//            return $response->withStatus(400)->withJson(['errors' => 'Attachment does not exist']); // EDISSYUM - NCH01 Fix typist des nouvelles version de PJ | Commenter cette ligne
            if (empty($queryParams['allowObs'])) { // EDISSYUM - NCH01 Fix typist des nouvelles version de PJ | Ajout du if
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        } elseif (!Validator::stringType()->notEmpty()->validate($body['type']) && !$onlyDocument) {
    ...
    public function setInSendAttachment(Request $request, Response $response, array $aArgs): Response
    {
        ...
        $signedAttachment = AttachmentModel::get([
            ...
            'where'  => ['res_id_master in (?)', 'status = ?', 'attachment_type = ?', 'origin = ?'],
            'data'   => [$attachment['res_id_master'], 'TRA', 'signed_response', "{$aArgs['id']},res_attachments"],
        ]);
    ...
    public function getFileContent(Request $request, Response $response, array $args): Response
    {
        ...
        // EDISSYUM - NCH01 Éviter les doublons de fichiers PDF convertis | Commenter les lignes suivantes
