...
class ListInstanceController
{
    ...
    public static function updateListInstance(array $args): array
    {
        ...
        $oldDestination = ResModel::getById(['select' => ['destination'], 'resId' => $args['data'][0]['resId']]);
        if ($oldDestination) {
            $oldDestination = $oldDestination['destination'];
        }

        $actionDestinations = ParameterModel::getById(['id' => 'ai_action_destination', 'select' => ['param_value_string']]); // EDISSYUM - NCH01 Ajout d'un filtre sur le userquota
        $actionDestinations = explode(',', $actionDestinations['param_value_string']);

        $newDestination = null;
        if (in_array($args['actionId'], $actionDestinations)) {
            if (!empty($args['data']) && !empty($args['data'][0]['destination'])) {
                $newDestination = $args['data'][0]['destination'];
                $destinationInfo = EntityModel::getById(['id' => $newDestination, 'select' => ['entity_id']]);
                if ($destinationInfo) {
                    $newDestination = $destinationInfo['entity_id'];
                }
            }
        }
        // END EDISSYUM - NCH01
        ...
        foreach ($args['data'] as $listInstanceByRes) {
            ...
            foreach ($listInstances as $listInstance) {
                ListInstanceHistoryDetailModel::create([
                    ...
                    'process_date'            => null,
                    'process_comment'         => null,
                    'old_destination'         => $listInstance['item_mode'] == 'dest' ? $oldDestination : null, // EDISSYUM - NCH01 Amélioration de l'historique de la listinstance pour l'IA
                    'ai_destination'         => $listInstance['item_mode'] == 'dest' ? $newDestination : null // EDISSYUM - NCH01 Amélioration de l'historique de la listinstance pour l'IA
                ]);
            }
        }
    ...
    public function updateCircuits(Request $request, Response $response, array $args): Response
    {
        ...
        foreach ($body['resources'] as $resourceKey => $resource) {
            ...
            if (
                (!array_key_exists('updateUser', $body) || !$body['updateUser']) && // EDISSYUM - LBE01 Correctif erreur désactivation utilisateur
            ...
            if (
                (!array_key_exists('updateUser', $body) || !$body['updateUser']) && // EDISSYUM - LBE01 Correctif erreur désactivation utilisateur
                $args['type'] == 'visaCircuit' &&
                (!empty($minimumVisaRole) || !empty($maximumSignRole))) {
                $nbVisaRole = 0;
                $nbSignRole = 0;
                foreach ($listInstances as $listInstance) {
