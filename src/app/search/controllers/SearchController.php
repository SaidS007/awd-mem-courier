...
class SearchController
{
    ...
    public static function getQuickFieldClause(array $args): array
    {
        ...
        if (!empty($body['meta']) && !empty($body['meta']['values']) && is_string($body['meta']['values'])) {
            if (
                ...
                and status in ('TRA', 'A_TRA', 'FRZ', 'SIGN') 
                and attachment_type <> 'summary_sheet')"; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
                ...
                if (!empty($requestDataAttachment['where'])) {
                    $whereClause[] = 'res_id in (select res_id_master from res_attachments where (' .
                        ...
                        ') and status in (\'TRA\', \'A_TRA\', \'FRZ\',\'SIGN\') and attachment_type <> \'summary_sheet\')'; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
                ...
                // EDISSYUM - NCH01 Rajout d'une recherche multi-critère
                if (!empty($body['multiCriteriaSearch'])) {
                    ...
                    if (!empty($requestDataContact['where'])) {
                        $whereClause[] = '(res_id IN (SELECT res_id FROM resource_contacts WHERE item_id IN (SELECT id FROM contacts WHERE ' . implode(' AND ', $requestDataContact['where']) . ') AND type = \'contact\'))';
                        $args['searchData'] = array_merge($args['searchData'], $requestDataContact['data']);
                    }

                    $fields = ['lastname', 'firstname', 'user_id'];
                    $fieldsNumber = count($fields);
                    $fields = AutoCompleteController::getInsensitiveFieldsForRequest(['fields' => $fields]);
                    $requestDataContact = AutoCompleteController::getDataForRequest([
                        'search'       => $quick,
                    ...
                    if (!empty($requestDataContact['where'])) {
                        $whereClause[] = '(res_id IN (SELECT res_id FROM resource_contacts WHERE item_id IN (SELECT id FROM users WHERE ' . implode(' AND ', $requestDataContact['where']) . ') AND type = \'user\'))';
                        $args['searchData'] = array_merge($args['searchData'], $requestDataContact['data']);
                    }
                }
                // END EDISSYUM - NCH01
    ...
    public static function getMainFieldsClause(array $args): ?array
    {
        ...
        // EDISSYUM - NCH01 Rajout d'une recherche multi-critère
        if (!empty($body['multiCriteria']) && !empty($body['multiCriteria']['values'])) {
            $value = is_string($body['multiCriteria']['values']) ? $body['multiCriteria']['values'] : $body['multiCriteria']['values'][0];
            $bodyTmp = [
                'meta' => [
                    'values' => $value
                ],
            ...
            $multiCriteriaResult = SearchController::getQuickFieldClause(['body' => $bodyTmp, 'searchWhere' => $args['searchWhere'], 'searchData' => $args['searchData']]);
            if (!empty($multiCriteriaResult)) {
                $args['searchWhere'] = $multiCriteriaResult['searchWhere'];
                $args['searchData']  = $multiCriteriaResult['searchData'];
            }
        }
        // END EDISSYUM - NCH01
        ...
        if (!empty($body['subject']) && !empty($body['subject']['values']) && is_string($body['subject']['values'])) {
            if (
                ...
                where title = ? and status in ('TRA', 'A_TRA', 'FRZ', 'SIGN') and attachment_type <> 'summary_sheet'))"; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
                ...
                where {$attachmentField} and status in ('TRA', 'A_TRA', 'FRZ', 'SIGN') and 
                attachment_type <> 'summary_sheet'))"; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
        ...
        if (!empty($body['chrono']) && !empty($body['chrono']['values']) && is_string($body['chrono']['values'])) {
            ...
            from res_attachments where identifier ilike ? and status in (\'TRA\', \'A_TRA\', \'FRZ\', \'SIGN\') 
            and attachment_type <> \'summary_sheet\'))'; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
        ...
        if (
            ...
            $args['searchData'][] = ['TRA', 'A_TRA', 'FRZ', 'SIGN']; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
        }
        if (
    ...
    public static function getFulltextClause(array $args): ?array
    {
        ...
        if (!empty($args['body']['fulltext']['values'])) {
            ...
            if (trim($args['body']['fulltext']['values']) === '') {
                throw new \Exception(_ERROR_MESSAGE_FULLTEXT_SEARCHED_VALUE_LENGTH);
            }
    ...
    private static function getAttachmentsInsider(array $args): array
    {
        ...
        if (!empty($body['meta']) && !empty($body['meta']['values']) && is_string($body['meta']['values'])) {
            if (
                ...
                where (title = ? OR identifier = ?) and status in ('TRA', 'A_TRA', 'FRZ', 'SIGN') 
                and attachment_type <> 'summary_sheet')"; // EDISSYUM - NCH01 Amélioration de la recherche de pièce jointe | Ajout SIGN'
                $data[] = $quick;
                $data[] = $quick;
            } else {
