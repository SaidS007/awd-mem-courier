...
class IndexingModelController
{
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        $model['forceCurrentUser']  = $model['force_current_user'] ?? false; // EDISSYUM - NCH01 Ajout d'un paramètre pour forcer l'utilisateur courant en tant qu'attributaire
        $model['noDiffusionRole']   = $model['no_diffusion_role'] ?? false; // EDISSYUM - NCH01 Ajout d'un paramètre pour ne pas remonter les rôles de diffusion dans l'indexation
        $model['sendMainDocParaph'] = $model['send_main_doc_paraph'] ?? false; // EDISSYUM - NCH01 Ajout d'un paramètre pour envoyer le document principal lors de l'envoi au visa
        $model['generateAr'] = $model['generate_ar'] ?? false; // EDISSYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement

        unset($model['mandatory_file']);
        unset($model['lad_processing']);
        unset($model['force_current_user']); // EDISSYUM - NCH01 Ajout d'un paramètre pour forcer l'utilisateur courant en tant qu'attributaire
        unset($model['no_diffusion_role']); // EDISSYUM - NCH01 Ajout d'un paramètre pour ne pas remonter les rôles de diffusion dans l'indexation
        unset($model['send_main_doc_paraph']); // EDISSYUM - NCH01 Ajout d'un paramètre pour envoyer le document principal lors de l'envoi au visa
        unset($model['generate_ar']); // EDISSYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        IndexingModelModel::update([
            'set'   => [
                ...
                'force_current_user'   => empty($body['forceCurrentUser']) ? 'false' : 'true', // EDISSYUM - NCH01 Ajout d'un paramètre pour forcer l'utilisateur courant en tant qu'attributaire
                'no_diffusion_role'    => empty($body['noDiffusionRole']) ? 'false' : 'true', // EDISSYUM - NCH01 Ajout d'un paramètre pour ne pas remonter les rôles de diffusion dans l'indexation
                'send_main_doc_paraph' => empty($body['sendMainDocParaph']) ? 'false' : 'true', // EDISSYUM - NCH01 Ajout d'un paramètre pour envoyer le document principal lors de l'envoi au visa
                'generate_ar'          => empty($body['generateAr']) ? 'false' : 'true', // EDISSYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement
            ],
            'where' => ['id = ?'],
            'data'  => [$args['id']]
        ]);
        ...
        if (!empty($allResourcesUsingModel)) {
            ...
            // EDISSYUM - PYB01 Fix des mises à jour des modèles d'enregistrement
            if (in_array('senders', $fieldsToDelete)) {
                foreach(array_chunk($allResourcesUsingModel, 1000) as $chunkOfAllResourcesUsingModel) {
                    ResourceContactModel::delete(['where' => ['res_id in (?)', 'mode = ?'], 'data' => [$chunkOfAllResourcesUsingModel, 'sender']]);
                }
            }
            if (in_array('recipients', $fieldsToDelete)) {
                foreach(array_chunk($allResourcesUsingModel, 1000) as $chunkOfAllResourcesUsingModel) {
                    ResourceContactModel::delete(['where' => ['res_id in (?)', 'mode = ?'], 'data' => [$chunkOfAllResourcesUsingModel, 'recipient']]);
                }
            }
            if (in_array('tags', $fieldsToDelete)) {
                foreach(array_chunk($allResourcesUsingModel, 1000) as $chunkOfAllResourcesUsingModel) {
                    ResourceTagModel::delete(['where' => ['res_id in (?)'], 'data' => [$chunkOfAllResourcesUsingModel]]);
                }
            }
            // END EDISSYUM - PYB01
        }
    ...
    public static function getIndexingModels(array $args): array
    {
        ...
        // EDISSYUM - NCH01 Fix récupération des modèles d'enregistrement pour le superadmin
        $currentUserMode = UserModel::getById(['id' => $GLOBALS['id'], 'select' => ['mode']]);
        if ($currentUserMode['mode'] == 'root_invisible') {
            $where = ['1 = 1'];
            $data = [];
        }
        // END EDISSYUM - NCH01
