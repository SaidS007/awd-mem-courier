...
class ConvertPdfController
{
    ...
    public static function getConvertedPdfById(array $args): mixed
    {
        ...
        if ($args['collId'] == 'letterbox_coll') {
            $resource = ResModel::getById(['resId' => $args['resId'], 'select' => ['version', 'format', 'docserver_id', 'filename', 'path', 'fingerprint']]); // EDISSYUM - NCH01 Éviter les doublons de fichiers PDF convertis | Ajout de 'format', 'docserver_id', 'filename', 'path', 'fingerprint'
            ...
            // EDISSYUM - NCH01 Éviter les doublons de fichiers PDF convertis
            $attachment = AttachmentModel::getById(['id' => $args['resId'], 'select' => ['format', 'docserver_id', 'filename', 'path', 'fingerprint']]);
            if (strtolower($attachment['format']) == 'pdf') {
                return $attachment;
            }
            // EDISSYUM - NCH01
