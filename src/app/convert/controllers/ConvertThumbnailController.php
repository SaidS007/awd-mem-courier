...
class ConvertThumbnailController
{
    ...
    public static function convertInThumbnail(string $filePath): array
    {
        ...
        if (in_array($ext, ['maarch', 'html'])) {
            ...
            // END EDISSYUM - NCH01
        } else if ($ext != 'zip') { // EDISSYUM - NCH01 Fix afin d'éviter la génération de miniatures pour les fichiers zip            $size = '750x900';
            ...
            $command = "HOME={$apache_tmp_dir} convert -thumbnail {$size} -background white -alpha remove "
                . escapeshellarg($filePath) . '[0] ' . escapeshellarg("{$tmpPath}{$fileNameOnTmp}.png");
            // END EDISSYUM - PYB01
        }
    ...
    public static function convert(array $aArgs): array|bool
    {
        ...
        if (empty($fileContent) && empty($extension) && empty($version)) {
            if ($aArgs['type'] == 'resource') {
                ...
                // EDISSYUM - NCH01 Fix de la génération de thumbnails des PJ à la volée
                else if (empty($convertedDocument)) {
                    $convertedDocument = AttachmentModel::getById([
                        'id' => $aArgs['resId'],
