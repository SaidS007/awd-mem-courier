...
use Throwable;
use SrcCore\models\DatabaseModel; // EDISSYUM - NCH01 IXBUS : Selection automatique du modèle de circuit
use Entity\models\ListInstanceModel; // EDISSYUM - NCH01 Rajout de la fonction processVisaWorkflow pour IXBUS
...
class IxbusController
{
     ...
     * @throws Exception
     */
    public static function getInitializeDatas($config, $resId): array // EDISSYUM - NCH01 IXBUS : Selection automatique du modèle de circuit | Ajout de $resId
    {
    ...
    public static function sendDatas($args): array
    {
        ...
        foreach ($attachments as $value) {
            ...
            if (!empty($args['config']['data']['optionSendOfficeDocument'] ?? null)) {
    ...
    public static function retrieveSignedMails($args): array
    {
        ...
        foreach ($args['idsToRetrieve'][$version] as $resId => $value) {
            ...
            if (in_array($folderData['data']['etat'], ['Refusé', 'Terminé'])) {
                ...
                // EDISSYUM - NCH01 Rajout de la fonction processVisaWorkflow pour IXBUS
                if ($folderData['data']['etat'] == 'Terminé') {
                    IxbusController::processVisaWorkflow(['res_id_master' => $value['res_id_master'], 'res_id' => $value['res_id'], 'processSignatory' => true]);
                }
                // END EDISSYUM - NCH01
            } else {
                if (!empty($folderData['error'])) {
