    ...
    public static function sendDatas($aArgs)
    {
        ...
        if (!empty($mainDocumentFilePath)) {
            $attachmentsData = [[
                ...
                'fileName' => TextFormatModel::formatFilename(['filename' => $mainResource['subject'], 'maxLength' => 90]) . '.pdf' // EDISSYUM - NCH01 Fix Blueway pour les noms de fichiers trop longs | Remplacer 250 par 90
            ]];
        }
        ...
        foreach ($attachments as $value) {
            ...
            $addedFile = BluewayController::addFileToFolder([
                ...
                'folderId' => $folderId,
                'filePath' => $filePath,
                'fileName' => TextFormatModel::formatFilename(['filename' => $value['title'], 'maxLength' => 90]) . '.pdf', // EDISSYUM - NCH01 Fix Blueway pour les noms de fichiers trop longs | Remplacer 250 par 90
                'fileType' => 'principal',
        ...
        if ($mainDocumentIntegration['inSignatureBook'] && empty($externalId['signatureBookId'])) {
            ...
            //$bodyData['nom'] = $mainResource['subject']; // EDISSYUM - NCH01 Fix Blueway pour les noms de fichiers trop longs | Commenter cette ligne
            $bodyData['nom'] = TextFormatModel::formatFilename(['filename' => $mainResource['subject'], 'maxLength' => 90]); // EDISSYUM - NCH01 Fix Blueway pour les noms de fichiers trop longs
