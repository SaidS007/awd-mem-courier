...
class PastellController
{
    ...
    public static function createDossier($aArgs) {
        ...
        // EDISSYUM - ASY01
        //récuperation des external Data
        if (isset($config['data']['visaSignature']->externalData->rules)) {
            $allValues = [];
            $tab = [];
            $sousType = "";
            foreach ($config['data']['visaSignature']->externalData->rules as $modelElements) {
                array_push($tab, (string)$modelElements->pastell_field);
                $type = (string)$modelElements->type;
                if ($type == 'text') {
                    array_push($tab, (string)$modelElements->value);
                }
                    ...
                    foreach (explode(',', $left_join) as $_ta) {
                        $left_joins[] = trim($_ta);
                    }
                    ...
                    $res = DatabaseModel::select([
                        'select' => [$select],
                        'table' => $tables,
                        'left_join' => $left_joins,
                        'where' => $whereArray,
                        'data' => [$aArgs['resIdMaster']]
                    ]);

                    if ($res and count($res)) {
                        array_push($tab, $res[0][$column]);
                    }
                }

                array_push($allValues, $tab);
                $tab = array_diff($tab, $tab);
            }

            $bodyDataDocument  = [];
            foreach ($allValues as $value) {
                $bodyDataDocument[$value[0]] = $value[1];
                if ($value[0] == "iparapheur_sous_type") {
                    $sousType = $value[1];
                }
            }
            ...
            if (!$sousTypeOK) {
                return ["error" => "Le sous type " . $sousType . " n'existe pas"];
            }
            ...
            if (!empty($curlResponse['error-message'])) {
                return ["error" => $curlResponse['error']];
            }
        }

        // Creation et ajout de metadata dans un fichier json meta.json
        if (isset($config['data']['visaSignature']->metadata->rules)) {
            $metadataTab = [];
            foreach ($config['data']['visaSignature']->metadata->rules as $modelElements) {
                array_push($tab, (string)$modelElements->pastell_field);
                $type = (string)$modelElements->type;
                if ($type == 'text') {
                    array_push($tab, (string)$modelElements->value);
                    ...
                    $res = DatabaseModel::select([
                        'select' => [$select],
                        'table' => $tables,
                        'left_join' => $left_join ? [$left_join] : [], // EDISSYUM - PYB01 Fix des jointures des paramètres Pastell
                        'where' => $whereArray,
                        'data' => [$aArgs['resIdMaster']]
                    ]);
            ...
            $bodyMetaData = array();
            foreach ($metadataTab as $value) {
                $bodyMetaData[$value[0]] = $value[1];
            }

            $jsonString = json_encode($bodyMetaData);
            $nomFichierTmp = tempnam("/tmp", "meta.json");
            file_put_contents($nomFichierTmp, $jsonString);

            $bodyData = array(
                'file_name' => 'metaa.json',
                'file_content' => file_get_contents($nomFichierTmp)
            );

            $curlResponse = CurlModel::exec([
                'url' => $aArgs['api_url'] . '/entite/' . $aArgs['id_e'] . '/document/' . $id_d . '/file/json_metadata',
                'basicAuth' => ['user' => $aArgs['config']['data']['userId'], 'password' => $aArgs['config']['data']['password']],
                'headers' => ['Content-Type' => 'application/x-www-form-urlencoded'],
                'method' => 'POST',
    ...
    public static function sendDatas($aArgs)
    {
        ...
        // EDISSYUM - PYB01 Fix affichage des erreurs dans pastell
        if (!empty($aArgs['error'])) {
            return $aArgs;
        }
        // END EDISSYUM - PYB01
        ...
        $annexes['letterbox'] = ResModel::get([
            ...
            'where' => ['res_id = ?', 'integrations->>\'inSignatureBook\' = \'true\''],
            'data' => [$aArgs['resIdMaster']]
        ]);
