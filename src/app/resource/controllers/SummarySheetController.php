...
use History\controllers\HistoryController; // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet
...
class SummarySheetController
{
    ...
    public function createList(Request $request, Response $response): Response
    {
        ...
        foreach ($resourcesByModelIds as $resourcesByModelId) {
            ...
            // Data for resources
            $sendSummarySheetDate = date('Y-m-d H:i:s');
            $data = SummarySheetController::prepareData(['units' => $units, 'resourcesIds' => $resourcesIds, 'sendSummarySheetDate' => $sendSummarySheetDate]);
            ...
            foreach ($resources as $resource) {
                SummarySheetController::createSummarySheet($pdf, [
                    ...
                    'sendSummarySheetDate' => $sendSummarySheetDate // EDISSYUM - (AMI01 + AMO01) Intégration fiche de liaison au parapheur externe - Ajout de sendSummarySheetDate
                ]);
            }
        }
    ...
    public static function createSummarySheet(Fpdi $pdf, array $args): void
    {
        ...
        $noteTypes = ['globalNotes', 'avisNotes', 'visaNotes']; // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet
        ...
        foreach ($units as $key => $unit) {
            ...
            if ($unit['unit'] == 'qrcode') {
                ...
                // EDISSYUM - NCH01 Rajout d'un paramètre pour choisir le contenu du QR Code de la fiche de liaison
                $data = $resource['res_id'];
                $qrCodeContent = ParameterModel::getById(['select' => ['param_value_string'], 'id' => 'summarySheetQrCodeContent']);
                if ($qrCodeContent && !empty($qrCodeContent['param_value_string'])) {
                    if (!empty($resource[$qrCodeContent['param_value_string']])) {
                        $data = $resource[$qrCodeContent['param_value_string']];
                    }
                }
                // END EDISSYUM - NCH01

                $qrCode = new QrCode($prefix . $data); // EDISSYUM - NCH01 Rajout d'un paramètre pour choisir le contenu du QR Code de la fiche de liaison | Remplacer $resource['res_id'] par $data                $qrCode->setSize(400);
                $qrCode->setMargin(25);
        ...
        foreach ($units as $key => $unit) {
            if ($unit['unit'] == 'primaryInformations') {
            ...
            } elseif (in_array($unit['unit'], $noteTypes)) { // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Remplacer $unit['unit'] == 'notes' par in_array($unit['unit'], $noteTypes)
                $noteType = $unit['unit']; // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet
                $notes = [];
                $found = false;
                $user = UserModel::getByLogin(['select' => ['id'], 'login' => $args['login']]);
                foreach ($args['data'][$noteType] as $noteKey => $rawNote) { // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Remplacer 'notes' par $noteType
                    if ($found && $rawNote['identifier'] != $resource['res_id']) {
                        ...
                        unset($args['data'][$noteType][$noteKey]); // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Remplacer 'notes' par $noteType
                        $found = true;
                    }
                }
                ...
                $listInstancesVisa = ListInstanceModel::get([
                ...
                if (isset($unit['sendSummarySheet']) && $unit['sendSummarySheet']) {
                    foreach ($listInstancesVisa as $key => $listInstance) {
                        if($listInstance['item_mode'] == 'visa' && empty($listInstance['process_date'])) {
                            $historyList[] = [
                                'event_date' => $args['sendSummarySheetDate'],
                                'user_id' => $GLOBALS['id'],
                                'info' => 'Visé et envoyé au parapheur externe',
                                'remote_ip' => ''
                            ];
                        }
                    }
        ...
        HistoryController::add([
            ...
            'eventType' => 'UP',
            'moduleId'  => 'resource',
            'eventId'   => 'summarySheetCreation',
            'info'      => _SUMMARY_SHEET_CREATION
        ]);
        // END EDISSYUM - AMO01
    }
    ...
    public static function prepareData(array $args): array
    {
        ...
        $data = [];
        $noteTypes = ['globalNotes' => "", 'avisNotes' => _AVIS . "]", 'visaNotes' => "[" . _VISA . "]"]; // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet
        foreach ($units as $unit) {
            $noteType = $unit['unit']; // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet
            if (in_array($noteType, array_keys($noteTypes))) { // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Changer la condition du if
                $data[$noteType] = NoteModel::get([
                ...
                // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet
                $data[$noteType] = array_filter($data[$noteType], function ($note) use ($noteType, $noteTypes) {
                ...
                // EDISSYUM - (AMI01 + AMO01) Intégration fiche de liaison au parapheur externe
                // Simulate the visa process for the external signature
                if (isset($unit['sendSummarySheet']) && $unit['sendSummarySheet']) {
                    foreach ($data['listInstancesVisa'] as $key => $listInstance) {
                        if($listInstance['item_mode'] == 'visa' && empty($listInstance['process_date'])) {
                            $data['listInstancesVisa'][$key]['process_date'] = $args['sendSummarySheetDate'];
                            $data['listInstancesVisa'][$key]['process_comment'] = 'Visé et envoyé au parapheur externe';
                        }
                    }
                }
            } elseif ($unit['unit'] == 'diffusionList') {
