...
class ResourceListController
{
    ...
    public static function getResourcesListQueryData(array $args): array
    {
        ...
        // EDISSYUM - NCH01 Amélioration des filtres de recherche dans les dossiers
        if (!empty($args['data']['order']) && strpos($args['data']['order'], 'subject') !== false) {
            $order = 'order_alphanum(subject) ' . explode(' ', $args['data']['order'])[1];
        }
        if (!empty($args['data']['order']) && strpos($args['data']['order'], 'creation_date') !== false) {
            $order = 'creation_date ' . explode(' ', $args['data']['order'])[1];
        }
        if (!empty($args['data']['order']) && strpos($args['data']['order'], 'status') !== false) {
            $order = 'order_alphanum(status) ' . explode(' ', $args['data']['order'])[1];
        }
        if (!empty($args['data']['order']) && strpos($args['data']['order'], 'type_label') !== false) {
            $order = 'order_alphanum(type_label) ' . explode(' ', $args['data']['order'])[1];
        }
        if (!empty($args['data']['order']) && strpos($args['data']['order'], 'entity_label') !== false) {
            $order = 'order_alphanum(entity_label) ' . explode(' ', $args['data']['order'])[1];
        }
        // END EDISSYUM - NCH01
    ...
    private static function getParallelOpinionsNumber(array $args): mixed
    {
        ...
        $notes = NoteModel::get(
            [
                'select' => ['count(1)'],
                'where'  => ['identifier = ?', 'note_text ilike ?'], // EDISSYUM - AMI01 Mise en majuscule de la notion d'annotation [AVIS] | Remplacer like par ilike
    ...
    public static function getFormattedResources(array $args): array
    {
        ...
        foreach ($resources as $key => $resource) {
            ...
            $formattedResources[$key]['processLimitDate'] = $resource['process_limit_date'] ?? null;
            $formattedResources[$key]['creationDate'] = $resource['creation_date'] ?? null;
    ...
    public static function getFormattedResourcesContacts(array $args) {
        ...
        $mapping = [
            ...
            'getAddressPostCode' => 'addresspostcode',
        ...
        foreach ($contacts as $key => $contact) {
            ...
            $formattedResources[$key]['email']              = $contact['email'] ?? '';
            $formattedResources[$key]['phone']              = $contact['phone'] ?? '';
            $formattedResources[$key]['addressNumber']      = $contact['addressnumber'] ?? '';
            $formattedResources[$key]['addressStreet']      = $contact['addressstreet'] ?? '';
            $formattedResources[$key]['addressAdditional1'] = $contact['addressadditional1'] ?? '';
            $formattedResources[$key]['addressAdditional2'] = $contact['addressadditional2'] ?? '';
            $formattedResources[$key]['addressPostcode']    = $contact['addresspostcode'] ?? '';
            $formattedResources[$key]['addressTown']        = $contact['addresstown'] ?? '';
            $formattedResources[$key]['addressCountry']     = $contact['addresscountry'] ?? '';
