...
//            $withSummarySheet = !empty($unitsSummarySheet) || !empty($resource['summarySheet']); // EDISSYUM - NCH01 Fix génération fiche de liaison dossier d'impression en masse | Commenter cette ligne

            $withSummarySheet = (!empty($unitsSummarySheet) || !empty($resource['summarySheet'])) && ($body['summarySheetMandatory'] || $body['summarySheet']); // EDISSYUM - NCH01 Fix génération fiche de liaison dossier d'impression en masse
            ...
            if (!empty($resource['document'])) {
                ...
                // EDISSYUM - NCH01 Amélioration du nommage des fichiers lors de l'impression
                $resource['subject']       = TextFormatModel::formatFilename(['filename' => $resource['subject'], 'maxLength' => 250]);
                $resource['subject']       = str_replace(["\r\n", "\n", "\r"], " ", $resource['subject']);
                // END EDISSYUM - NCH01
            ...
            if (!empty($resource['attachments'])) {
                if (is_array($resource['attachments'])) {
                    ...
                    $attachments = AttachmentModel::get([
                        ...
                        'where'   => ['res_id in (?)', 'status not in (?)', 'status <> ?', 'attachment_type not in (?)'], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'status <> ?', 'attachment_type <> ?' | AMI01 Amélioration de l'impression de la fiche de liaison - Ajout du type 'summary_sheet'
                        'data'    => [$resource['attachments'], ['DEL', 'OBS'], 'SIGN', ['signed_response', 'summary_sheet']], // EDISSYUM - AMI01 Amélioration de l'impression de la fiche de liaison - Ajout du type 'summary_sheet'
                    ...
                    $attachments = AttachmentModel::get([
                        ...
                        'where'   => ['res_id_master = ?', 'status not in (?)', 'status <> ?', 'attachment_type not in (?)'], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'status <> ?', 'attachment_type <> ?' AMI01 Amélioration de l'impression de la fiche de liaison - Ajout du type 'summary_sheet'
                        'data'    => [$resource['resId'], ['DEL', 'OBS'], 'SIGN', ['signed_response', 'summary_sheet']], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'SIGN', 'signed_response' AMI01 Amélioration de l'impression de la fiche de liaison - Ajout du type 'summary_sheet'
                        'orderBy' => ['creation_date desc']
                    ]);
                }
            ...
            if (!empty($resource['attachmentsSigned'])) {
                $attachments = AttachmentModel::get([
            ...
            // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Revoir la gestion complète des notes en faisant un foreach des noteTypes
...
//            }
            ...
            $noteTypes = ['globalNotes' => "", 'avisNotes' => _AVIS . "]", 'visaNotes' => "[" . _VISA . "]"];

            foreach (array_keys($noteTypes) as $noteType) {
                if (!empty($resource[$noteType])) {
                    if (is_array($resource[$noteType])) {
                        foreach ($resource[$noteType] as $note) {
                        ...
                        if (count($notes) < count($resource[$noteType])) {
                            return $response->withStatus(400)->withJson(['errors' => 'Note(s) not found']);
                        }
                    } else {
                        $notes = NoteModel::getByUserIdForResource([
                    ...
                    if (!empty($notes)) {
                        $notes = array_filter($notes, function ($note) use ($noteType, $noteTypes) {
                            if (in_array($noteType, ['visaNotes', 'avisNotes'])) {
                                return str_contains($note['note_text'], $noteTypes[$noteType]);
                            }
                            return !str_contains($note['note_text'], $noteTypes['avisNotes']) &&
                                !str_contains($note['note_text'], $noteTypes['visaNotes']);
                        });

                        $noteFilePath = FolderPrintController::getNotesFilePath(['notes' => $notes, 'resId' => $resource['resId'], 'noteType' => $noteType]);
            ...
            if (!empty($documentPaths)) {
                ...
                // EDISSYUM - NCH01 Sanitize filename pour dossier d'impression
                $filePathOnTmp = str_replace(' ', '_', $filePathOnTmp);
                $filePathOnTmp = str_replace("'", '', $filePathOnTmp);
                $filePathOnTmp = str_replace('"', '', $filePathOnTmp);
                // END EDISSYUM - NCH01

                $tmpFiles = []; // EDISSYUM - NCH01 Fix dossier d'impression avec PDF chiffré
                foreach ($documentPaths as $key => $documentPath) {

                    $filename = pathinfo($documentPath, PATHINFO_FILENAME);
                    // EDISSYUM - AMI01 Nettoyage des PDF pour dossier d'impression
                    $documentPath = $tmpDir . $filename . '_cleaned.pdf';
                    if (file_exists($documentPath)) {
                        unlink($documentPath);
                    }
                    $command = "gs -sDEVICE=pdfwrite -dNOPAUSE -dBATCH -dSAFER -sOutputFile=$documentPath $documentPaths[$key]";
                    exec($command . ' 2>&1', $output, $return);

                    // EDISSYUM - NCH01 Fix dossier d'impression avec PDF chiffré |  AMI01 Nettoyage des PDF pour dossier d'impression
                    if (self::isPdfEncrypted($documentPath)) {
                        $documentPath = str_replace('_cleaned.pdf', '_decrypted.pdf', $documentPath);
                        $command = "qpdf --decrypt '" . $documentPaths[$key] . "' '" . $documentPath . "'";
                        exec($command . ' 2>&1', $output, $return);

                        if ($return != 0) {
                            return $response->withStatus(500)->withJson(['errors' => 'PDF file is encrypted']);
                        }
                    }
                    // END EDISSYUM - NCH01
                    $tmpFiles[] = $documentPath;
                    // END EDISSYUM - AMI01
                ...
                $command = "pdfunite '" . implode("' '", $tmpFiles) . "' '" . $filePathOnTmp . "'"; // EDISSYUM - AMI01 Nettoyage des PDF pour dossier d'impression
                exec($command . ' 2>&1', $output, $return);
                ...
                // EDISSYUM - NCH01 Fix dossier d'impression avec PDF chiffré
    ...
    // EDISSYUM - NCH01 Fix dossier d'impression avec PDF chiffré
    private function isPdfEncrypted($filename): bool
    {
    ...
    private static function getNotesFilePath(array $args): array|string
    {
        ...
        if (!isset($args['noteType'])) {
            $args['noteType'] = 'globalNotes';
        }

        $annotationsTitles = [
            'avisNotes' => _AVIS_NOTES_COMMENT,
            'visaNotes' => _VISA_NOTES_COMMENT,
            'globalNotes' => _GLOBAL_NOTES_COMMENT
        ];
        // END EDISSYUM - AMO01
        ...
        $pdf->SetFont('', 'B', 11);
        $pdf->Cell(0, 15, $annotationsTitles[$args['noteType']], 0, 2, 'L', false); // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Remplacer _NOTES_COMMENT par$annotationsTitles[$args['noteType']]
        ...
        $tmpDir = CoreConfigModel::getTmpPath();
        $filePathOnTmp = $tmpDir . 'list' . $args['noteType'] . '_' . $GLOBALS['id'] . '.pdf'; // EDISSYUM - AMO01 Dissocier les annotations par type + Adapter le paramètre defaultStatusSummarySheet | Remplacer 'listNotes_' par 'list' . $args['noteType'] . '_'
