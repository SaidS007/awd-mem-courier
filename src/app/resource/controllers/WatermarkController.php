    ...
    public static function watermarkResource(array $args): ?string
    {
        ...
        foreach ($matches[1] as $value) {
            if ($value == 'date_now') {
                ...
                $tmp_value = trim(preg_replace('/TO_CHAR.*as/', '', $value)); // EDISSYUM - NCH01 Amélioration des watermarks pour intégrer TO_CHAR
                $tmp = $resource[$tmp_value] ?? ''; // EDISSYUM - NCH01 Amélioration des watermarks pour intégrer TO_CHAR | remplacer [$tmp] par [$tmp_values]
            }
