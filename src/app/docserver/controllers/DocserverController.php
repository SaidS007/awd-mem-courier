...
class DocserverController
{
    ...
    public function update(Request $request, Response $response, array $aArgs): Response
    {
        ...
        DocserverModel::update([
            'set'   => [
                ...
                'is_encrypted'      => !$data['is_encrypted'] ? 'false' : 'true' // EDISSYUM - NCH01 Fix de la modification des docservers depuis l'interface | Remplacer $data['is_encrypted'] ?? false par !$data['is_encrypted'] ? 'false' : 'true'
            ],
            'where' => ['id = ?'],
            'data'  => [$aArgs['id']]
        ]);
    ...
    public static function createPathOnDocServer(array $args): array
    {
        ...
        if (!empty($GLOBALS['wb'])) {
            ...
            if (!is_dir($pathToDS)) {
                ...
                umask(0022);
                chmod($pathToDS, 0775); // EDISSYUM - PYB01 Fix docservers rights for VPS
            }
        }
    ...
    public static function getNextFileNameInDocServer(array $aArgs): array
    {
        ...
        if ($filesNb == 0) {
            ...
            if (!mkdir($zeroOnePath, 0775)) {// EDISSYUM - PYB01 Fix docservers rights for VPS
            ...
            if ($filesNbBis >= 1000) { //If number of files >= 1000 then creates a new subdirectory
                ...
                if (!mkdir($zeroNumberPath, 0775)) { // EDISSYUM - PYB01 Fix docservers rights for VPS
    ...
    public static function copyOnDocServer(array $aArgs): array
    {
        ...
        if (!is_dir($aArgs['destinationDir'])) {
            mkdir($aArgs['destinationDir'], 0775, true); // EDISSYUM - PYB01 Fix docservers rights for VPS
            if (DIRECTORY_SEPARATOR == '/' && !empty($GLOBALS['apacheUserAndGroup'])) {
                exec(
                    'chown ' . escapeshellarg($GLOBALS['apacheUserAndGroup']) . ' ' .
