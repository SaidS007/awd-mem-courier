...
class ContactGroupController
{
    ...
    public function getCorrespondentsById(Request $request, Response $response, array $args): Response
    {
        ...
        !is_numeric($queryParams['limit']) ? $limit : (int)$queryParams['limit']); // EDISSYUM - NCH01 Ajout d'un paramètre pour gérer le nombre max lors d'un publipostage | Changer le 250 en $limit
        ...
        if (!empty($queryParams['search'])) {
            $fields = [
                ...
                'contacts_groups.label' // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre | Ajouter contacts_groups.label
            ];
        ...
        foreach ($rawCorrespondents as $correspondent) {
            if ($correspondent['correspondent_type'] == 'contact') {
                // EDISSYUM - NCH01 Ne pas récupérer les contacts disabled dans les groupes de correspondants depuis l'autocomplete
                $where = ['id = ?'];
                $data = [$correspondent['correspondent_id']];
                if (isset($queryParams['fromProcess']) && $queryParams['fromProcess']) {
                    $where[] = 'enabled = ?';
                    $data[] = 1;
                }
                // END NCH01

                // EDISSYUM - NCH01 Ne pas récupérer les contacts disabled dans les groupes de correspondants depuis l'autocomplete | Commenter ce bloc
...
//                ]);
                ...
                if (!empty($contact)) {
                    $contact = $contact[0]; // EDISSYUM - NCH01 Ne pas récupérer les contacts disabled dans les groupes de correspondants depuis l'autocomplete
                    ...
                    $correspondents[] = [
                        ...
                        'enabled'           => $contact['enabled'] // EDISSYUM - NCH01 Améliorer l'affichage des utilisateurs suspendus dans les groupements de contacts
                    ];
                }
    ...
    public function addCorrespondents(Request $request, Response $response, array $args): Response
    {
        ...
        foreach ($body['correspondents'] as $correspondent) {
            ...
            if (
                ...
                in_array($correspondent['type'], [
                    'contact',
                    'user',
                    'entity',
                    'contactGroup' // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre | Ajout de contactGroup dans le in_array
                ])
            ) {
                ContactGroupListModel::create([
