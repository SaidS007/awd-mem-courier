...
use Contact\models\ContactGroupModel; // EDISSYUM - NCH01 Ajout des groupements de correspondants dans l'export des contacts
...
class ContactController
{
    ...
    public function get(Request $request, Response $response): Response
    {
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'admin_contacts', 'userId' => $GLOBALS['id']])
            && !PrivilegeController::hasPrivilege(['privilegeId' => 'view_contacts', 'userId' => $GLOBALS['id']])) // EDISSYUM - NCH01 Rajout de la possibilité de visualiser les contacts sans droits d'administration | dans le if, rajout du && ......
        {
        ...
        if (!empty($queryParams['search'])) {
            $fields = [
                ...
                'address_town',
                'address_country',
                'email' // EDISSYUM - NCH01 Module formulaire SVE | ajout email
            ];
        ...
        if (isset($queryParams['where']) && (!isset($requestData['where']) ||  !$requestData['where'])) { //EDISSYUM - LBE01 Correctif recherche contacts | utiliser isset
            $requestData['where'] = [$queryParams['where']];
        }
        // END EDISSYUM - NCH01

        // EDISSYUM - NCH01 Module OCForMEM NextGen
        $requestData['where'][] = "status <> 'TMP'";
        // END EDISSYUM - NCH01
        ...
        $contacts = ContactModel::get([
            'select'  => [
                ...
                'count(1) OVER()',
                'email', // EDISSYUM - NCH01 Module formulaire SVE | ajout email
                'custom_fields', // EDISSYUM - NCH01 Rajout de la confidentialité des contacts ajout de custom_fields
                'external_id as "externalId"' // EDISSYUM - NCH01 Module OCForMEM NextGen
            ],
        ...
        foreach ($contacts as $key => $contact) {
            ...
            $contacts[$key] = ContactController::getContactConfidentiality($contact); // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        if (!empty($body['email']) && $currentUser['mode'] == 'rest') {
            ...
            if (!empty($contact[0]['id'])) {
                // EDISSYUM - NCH01 Module OCForMEM NextGen
                $allowDuplicateMail = $request->getQueryParams()['allowDuplicateMail'];
                if (empty($allowDuplicateMail) || $allowDuplicateMail !== 'true') {
        ...
        $id = ContactModel::create([
            'status'                => $body['status'] ?? 'OK', // EDISSYUM - NCH01 Module OCForMEM NextGen
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        $contact = [
            'id'                 => $rawContact['id'],
            'civility'           => null,
            'status'                => $rawContact['status'], // EDISSYUM - NCH01 Module OCForMEM NextGen
        ...
        $contact = ContactController::getContactConfidentiality($contact);  // EDISSYUM - NCH01 Rajout de la confidentialité des contacts

        return $response->withJson($contact);
    }

    // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
    public static function getContactConfidentiality($contact, $returnNoneIfNameConfidential = false) {
        $confidentialitySettings = \Parameter\models\ParameterModel::getById(['select' => ['param_value_string'], 'id' => 'contactsConfidentiality']);

        $user = UserModel::getById(['id' => $GLOBALS['id'], 'select' => ['mode']]);
        ...
        $userPrivileges = \Group\controllers\PrivilegeController::getPrivilegesByUser(['userId' => $GLOBALS['id']]);
        $userPrivilegesBasic = array_filter($userPrivileges, function($privilege) {
            return $privilege == 'view_confidential_contact' || $privilege == 'ALL_PRIVILEGES';
        });
        $userPrivilegesAdvanced = array_filter($userPrivileges, function($privilege) {
            return $privilege == 'view_confidential_contact_advanced' || $privilege == 'ALL_PRIVILEGES';
        });

        if ($confidentialitySettings) {
            $settings = json_decode($confidentialitySettings['param_value_string']);

            $basicConfidentiality = false;
            $advancedConfidentiality = false;

            $basicCustomId = $settings -> basic -> customId ?? null;
            $advancedCustomId = $settings -> advanced -> customId ?? null;
            $basicHiddenFields = array_map('trim', explode(',', $settings -> basic -> hiddenFields)) ?? null;
            $basicEntitiesAllowed = array_map('trim', explode(',', $settings -> basic -> entitiesAllowed)) ?? null;
            $advancedHiddenFields = array_map('trim', explode(',', $settings -> advanced -> hiddenFields)) ?? null;
            $advancedEntitiesAllowed = array_map('trim', explode(',', $settings -> advanced -> entitiesAllowed)) ?? null;

            if (!empty($basicHiddenFields) && in_array('address', $basicHiddenFields)) {
                $basicHiddenFields = array_merge($basicHiddenFields, ['addressNumber', 'addressStreet', 'addressAdditional1', 'addressAdditional2', 'addressPostcode', 'addressTown', 'addressCountry']);
                $basicHiddenFields = array_merge($basicHiddenFields, ['address_number', 'address_street', 'address_additional1', 'address_additional2', 'address_postcode', 'address_town', 'address_country']);
            }
            if (!empty($advancedHiddenFields) && in_array('address', $advancedHiddenFields)) {
                $advancedHiddenFields = array_merge($advancedHiddenFields, ['addressNumber', 'addressStreet', 'addressAdditional1', 'addressAdditional2', 'addressPostcode', 'addressTown', 'addressCountry']);
                $advancedHiddenFields = array_merge($advancedHiddenFields, ['address_number', 'address_street', 'address_additional1', 'address_additional2', 'address_postcode', 'address_town', 'address_country']);
            }

            if ((!array_key_exists('custom_fields', $contact) || empty($contact['custom_fields'])) && array_key_exists('customFields', $contact) && !empty($contact['customFields'])) {
                $contact['custom_fields'] = $contact['customFields'];
            }

            if (empty($contact['custom_fields'])) {
                $contact['custom_fields'] = ContactModel::getById(['id' => $contact['id'], 'select' => ['custom_fields']])['custom_fields'] ?? '{}';
            }
            ...
            if ($contact['custom_fields']) {
                foreach ($contact['custom_fields'] as $key => $value) {
                    if ($key == $basicCustomId && $value == _YES) {
                        $basicConfidentiality = true;
                        if (!empty($userPrivilegesBasic)) {
                            if ($basicEntitiesAllowed[0]) {
                                foreach ($userEntities as $userEntity) {
                                    if (in_array($userEntity, $basicEntitiesAllowed)) {
                                        $basicConfidentiality = false;
                                    }
                                }
                            } else {
                    ...
                    } elseif ($key == $advancedCustomId  && $value == _YES) {
                        $advancedConfidentiality = true;
                        if (!empty($userPrivilegesAdvanced)) {
                            if ($advancedEntitiesAllowed[0]) {
                                foreach ($userEntities as $userEntity) {
                                    if (in_array($userEntity, $advancedEntitiesAllowed)) {
                                        $advancedConfidentiality = false;
                                    }
                                }
                            } else {
            ...
            $lastnameOrFirstnameConf = false;
            if ($basicConfidentiality || $advancedConfidentiality) {
                foreach ($contact as $key => $value) {
                    if ($basicConfidentiality) {
                        if ($key == 'custom_fields') {
                            foreach ($value as $customFieldKey => $customFieldValue) {
                                $customName = 'custom_' . $customFieldKey;
                                if (in_array($customName, $basicHiddenFields)) {
                                    $contact[$key][$customFieldKey] = _CONFIDENTIALITY;
                                }
                            }
                        } elseif (in_array($key, $basicHiddenFields)) {
                            if ($key == 'lastname' || $key == 'firstname') {
                                $lastnameOrFirstnameConf = true;
                            }
                            $contact[$key] = _CONFIDENTIALITY;
                        }
                    } elseif ($advancedConfidentiality) {
                        if ($key == 'custom_fields') {
                            foreach ($value as $customFieldKey => $customFieldValue) {
                                $customName = 'custom_' . $customFieldKey;
                                if (in_array($customName, $advancedHiddenFields)) {
                                    $contact[$key][$customFieldKey] = _CONFIDENTIALITY;
                                }
                            }
                        } else if (in_array($key, $advancedHiddenFields)) {
                            if ($key == 'lastname' || $key == 'firstname') {
                                $lastnameOrFirstnameConf = true;
                            }
                            $contact[$key] = _CONFIDENTIALITY;
            ...
            if ($returnNoneIfNameConfidential && $lastnameOrFirstnameConf) {
                $contact = null;
            }
        }
        return $contact;
        ...
        foreach ($data['contactsParameters'] as $key => $contactParameter) {
            ...
            ContactParameterModel::update([
                'set'   => [
                    ...
                    'exportable' => empty($contactParameter['exportable']) ? 'false' : 'true' // EDISSYUM - ASY01 Ajout de modèle d'exportation PDF/CSV
                ],
                'where' => ['id = ?'],
    ...
    public function getDuplicatedContacts(Request $request, Response $response): Response
    {
        ...
        foreach ($queryParams['criteria'] as $criterion) {
            if ($criterion === 'civility') {
                ...
                $order[] = $allowedFields[$criterion];
                $criteria[] = "replace(lower(unaccent(" . $allowedFields[$criterion] . "::TEXT) ), ' ', '')"; // EDISSYUM - OBR01 Fix de type d'argument pour la fonction unaccent | Ajout de ::TEXT
            }
        }
        ...
        $where[] = "status NOT IN ('TMP')"; // EDISSYUM - NCH01 Module OCForMEM NextGen
    ...
    public function exportContacts(Request $request, Response $response): Response
    {
        ...
        foreach ($body['data'] as $field) {
            ...
            } elseif (!in_array($field['value'], $existingFieldsKeys) && $field['value'] != 'creatorLabel' && $field['value'] != 'contactGroup') { // EDISSYUM - NCH01 Ajout des groupements de correspondants dans l'export des contacts | Ajout de && $field['value'] != 'contactGroup'
        ...
        foreach ($body['data'] as $field) {
            ...
            } // EDISSYUM - NCH01 Ajout des groupements de correspondants dans l'export des contacts
            elseif ($field['value'] == 'contactGroup') {
                $fields[] = 'contacts.id as contact_group';
            }
            // END EDISSYUM - NCH01
            else {
        ...
        foreach ($contacts as $contact) {
            ...
            // EDISSYUM - NCH01 Ajout des groupements de correspondants dans l'export des contacts
            if (!empty($contact['contact_group'])) {
                $groups = '';
                $contactsGroups = ContactGroupModel::getWithList(['where' => ['correspondent_id = ?'], 'data' => [$contact['contact_group']]]);
                foreach ($contactsGroups as $contactsGroup) {
                    $groups .= $contactsGroup['label'] . PHP_EOL;
                }
                $contact['contact_group'] = rtrim($groups, PHP_EOL);
            }
            // END EDISSYUM - NCH01
        ...
        $response = $response->withAddedHeader(
            'Content-Disposition',
            'attachment; filename=export_mem.csv'
        );
    ...
    public static function getParsedContacts(array $args): array
    {
        ...
        // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
        foreach ($contacts as $key => $contact) {
            $contacts[$key] = ContactController::getContactConfidentiality($contact);
        }
        // END EDISSYUM - NCH01
    ...
    public static function getAutocompleteFormat(array $args): array
    {
        ...
        // EDISSYUM - NCH01 Module OCForMEM NextGen
        $displayableStdParameters[] = 'status';
        $displayableStdParameters[] = 'external_id';
        // END EDISSYUM - NCH01
        ...
        // EDISSYUM - NCH01 Module OCForMEM NextGen
        if (in_array('status', $displayableStdParameters)) {
            $contact['status'] = $rawContact['status'];
        }
        if (in_array('external_id', $displayableStdParameters)) {
            $contact['externalId'] = json_decode($rawContact['external_id']);
        }
        // END EDISSYUM - NCH01
     ...
     * @throws Exception
     */
    public function getByPhone(Request $request, Response $response): Response {
     ...
     * @throws Exception
     */
    public function getByMail(Request $request, Response $response):Response {
        $data = $request->getQueryParams();
    ...
    public function getResourcesByContactId(Request $request, Response $response, array $args) {
        ...
        if (!empty($queryParams['select'])) {
            $select = array_merge($select, explode(',', $queryParams['select']));
        }
