...
import { ConfirmComponent } from "@plugins/modal/confirm.component"; // EDISSYUM - NCH01 Module OCForMEM NextGen
import { ProcessComponent } from "@appRoot/process/process.component"; // EDISSYUM - NCH01 Module OCForMEM NextGen
...
@Component({
    ...
    providers: [SortPipe, ContactService, {
            provide: ProcessComponent,
            useValue: window
        }] // EDISSYUM - NCH01 Module OCForMEM NextGen | Ajout de la déclaration du service ProcessComponent
    ,
    standalone: false
})
...
export class ContactAutocompleteComponent implements OnInit {
    ...
    @Input() fromIndexingForm: boolean = false; // EDISSYUM - NCH01 Module OCForMEM NextGen
    @Input() externalId: any = []; // EDISSYUM - NCH01 Module OCForMEM NextGen
        ...
        this.controlAutocomplete.value.forEach((contact: any) => {
            if (!this.functions.empty(contact) && typeof contact === 'object'){
                this.valuesToDisplay[contact.type][contact.id] = {
                    type: '',
                    status: '', // EDISSYUM - NCH01 Module OCForMEM NextGen
                    externalId: {}, // EDISSYUM - NCH01 Module OCForMEM NextGen
                    firstname: '',
                    ...
                    this.http.get('../rest/contacts/' + contact.id).pipe(
                        tap((data: any) => {
                            this.valuesToDisplay['contact'][data.id] = {
                                type: 'contact',
                                status: data.status, // EDISSYUM - NCH01 Module OCForMEM NextGen
                                externalId: data.externalId, // EDISSYUM - NCH01 Module OCForMEM NextGen
    ...
    setFormValue(item: any) {
        if (item.type === 'contactGroup') {
            this.http.get('../rest/contactsGroups/' + item.id + '/correspondents?limit=none&fromProcess=true').pipe( // EDISSYUM - NCH01 Ne pas récupérer les contacts disabled dans les groupes de correspondants depuis l'autocomplete | ajout &fromProcess=true
                map((data: any) => {
                    const contacts = data.correspondents.map((correspondent: any) => ({
                        id: correspondent.id,
                        type: correspondent.type,
                        status: correspondent.status, // EDISSYUM - NCH01 Module OCForMEM NextGen
                        externalId: correspondent.externalId, // EDISSYUM - NCH01 Module OCForMEM NextGen
                ...
                tap((contacts: any) => {
                    contacts.forEach((contact: any) => {
                        ...
                        if (contact.type === 'contactGroup') {
                            this.http.get(`../rest/contactsGroups/${contact.id}/correspondents?fromProcess=true`).pipe( // EDISSYUM - NCH01 Ne pas récupérer les contacts disabled dans les groupes de correspondants depuis l'autocomplete | ajout ?fromProcess=true
                                tap((data: any) => {
    ...
    setContact(contact: any) {
        if (this.controlAutocomplete.value.filter((contactItem: any) => contactItem.id === contact.id && contactItem.type === contact.type).length === 0) {
            ...
            arrvalue.push(
                {
                    status : contact['status'], // EDISSYUM - NCH01 Module OCForMEM NextGen
                    externalId : contact['externalId'], // EDISSYUM - NCH01 Module OCForMEM NextGen
                    type: contact['type'],
                    id: contact['id'],
    ...
    removeItem(index: number, contact: any = null) {
        // EDISSYUM - NCH01 Implémentation SerenIA
        if (contact) {
            index = this.controlAutocomplete.value.findIndex((item: any) => item.id === contact.id && item.type === contact.type);
        }
        // END EDISSYUM - NCH01

        // EDISSYUM - NCH01 Module OCForMEM NextGen
        if (this.fromIndexingForm) {
            if (this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id]) {
                if (this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].status === 'TMP' ||
                    (this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId &&
                        this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId['ia_tmp_contact_id']) && this.externalId && this.externalId['ia_tmp_contact_id']
                    && this.externalId['ia_tmp_contact_id'] === this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId['ia_tmp_contact_id']) {
                    this.dialogRef = this.dialog.open(ConfirmComponent, {
                        panelClass: 'maarch-modal',
                        autoFocus: false,
                        disableClose: true,
                        data: {
                            title: this.translate.instant('lang.deleteTmpContact'),
                            msg: this.translate.instant('lang.deleteTmpContactMsg')
                        }
                    });
                    ...
                    this.dialogRef.afterClosed().pipe(
                        tap((data: any) => {
                            if (data === 'ok') {
                                let contact_id = this.controlAutocomplete.value[index].id;

                                if (this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId &&
                                    this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId['ia_tmp_contact_id']) {
                                    const tmp_contact_id = this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId['ia_tmp_contact_id'];

                                    delete this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id].externalId['ia_tmp_contact_id'];
                                    const finalContact = this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id];
                                    this.http.put(`../rest/contacts/${contact_id}`, finalContact).pipe(
                                        catchError((err: any) => {
                                            this.notify.handleErrors(err);
                                            return of(false);
                                        })
                                ...
                                delete this.valuesToDisplay['contact'][this.controlAutocomplete.value[index].id];
                                this.http.delete('../rest/contacts/' + contact_id).pipe(
                                    catchError((err: any) => {
                                        this.notify.handleErrors(err);
                                        return of(false);
                                    })
        ...
        // EDISSYUM - NCH01 Module OCForMEM NextGen | Commenter le code
        // this.removeContactEvent.emit(this.controlAutocomplete.value[index].id);
        //
        // if (this.newIds.indexOf(this.controlAutocomplete.value[index]) === -1) {
        //     const arrValue = this.controlAutocomplete.value;
    ...
    openContact(contact: any = null, updateTmpContact: boolean= false) { // EDISSYUM - NCH01 Module OCForMEM NextGen | Rajout de updateTmpContact
        // EDISSYUM - NCH01 Module OCForMEM NextGen
        if (updateTmpContact && !this.privilegeService.hasCurrentUserPrivilege('update_contacts')) {
            this.notify.error(this.translate.instant('lang.noPrivileges'));
            return;
        }

        if (contact) {
            const tmp_c = this.valuesToDisplay['contact'][contact['id']];
            if (!updateTmpContact && tmp_c && (tmp_c['status'] === 'TMP' ||
                (tmp_c['externalId'] && tmp_c['externalId']['ia_tmp_contact_id'] && this.externalId && this.externalId['ia_tmp_contact_id']
                    && this.externalId['ia_tmp_contact_id'] === tmp_c['externalId']['ia_tmp_contact_id']))) {
                this.notify.error(this.translate.instant('lang.canotEditTmpContact'));
                return;
            }
        }
        // END EDISSYUM - NCH01
        ...
        const dialogRef = this.dialog.open(
            ContactModalComponent,
            {
                ...
                disableClose: true,
                data: {
                    updateTmpContact: updateTmpContact, // EDISSYUM - NCH01 Module OCForMEM NextGen
