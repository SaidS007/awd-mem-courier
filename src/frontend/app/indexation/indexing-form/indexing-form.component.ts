...
export class IndexingFormComponent implements OnInit {
            ...
            this.http.get(route).pipe(
                tap((data: any) => {
                    ...
                    if (this.adminMode) {
                        ...
                        if (elem.default_value === '#myPrimaryEntity' || this.indexingModelClone.forceCurrentUser) { // EDISSYUM - NCH01 Ajout d'un paramètre pour forcer l'utilisateur courant en tant qu'attributaire | Ajout de this.indexingModelClone.forceCurrentUser
                            this.selfDest = this.currentCategory === 'outgoing' || this.indexingModelClone.forceCurrentUser; // EDISSYUM - NCH01 Ajout d'un paramètre pour forcer l'utilisateur courant en tant qu'attributaire | Ajout de this.indexingModelClone.forceCurrentUser
                        ...
                        await Promise.all(this['indexingModels_' + element].map(async (elem: any) => {
                            ...
                            if (Object.keys(data).indexOf(elem.identifier) > -1 || customId !== undefined) {
                                ...
                                if (!this.functions.empty(fieldValue)) {
                                    ...
                                    // EDISSYUM - NCH01 Afficher le message suggestLinksNdaysAgo dès l'entrée dans le formulaire
                                    if (elem.identifier === 'senders') {
                                        if (fieldValue.length == 1) {
                                            this.selectedContact(fieldValue[0], elem.identifier);
                                        }
                                    }
                                    // END EDISSYUM - NCH01
                                }
                            ...
                            // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut
                            if (this.canChangeModel && elem.default_value && elem.default_value.length !== 0 && this.indexingFormId !== data.modelId) {
                                this.arrFormControl[elem.identifier].setValue(elem.default_value);
                            }
                            //  END EDISSYUM - NCH01
    ...
    calcLimitDate(field: any, value: any) {
        return new Promise((resolve) => {
            ...
            const objToSend: any = {
                doctype: value,
                // priority: this.arrFormControl['priority']?.value // EDISSYUM - NCH01 Fix modification de la priorité en fonction du type de document | Commenter cette ligne
            };
            // EDISSYUM - NCH01 Fix modification de la priorité en fonction du type de document | Commenter ce bloc if
