...
export class EditorManagerComponent implements OnInit {
    ...
    async loadResource(): Promise<boolean> {
        return new Promise((resolve) => {
            const resourceUrl = `../rest/${this.isAttachment ? 'attachments' : 'resources'}/${this.resId}/originalContent?mode=base64`;
    ...
    saveFile(): void {
        ...
        this.editorComp?.getFile().pipe(
            ...
            exhaustMap((data) => this.http.put(`../rest/${this.isAttachment ? 'attachments' : 'resources'}/${this.resId}?onlyDocument=true`, data)),
            tap(() => {
