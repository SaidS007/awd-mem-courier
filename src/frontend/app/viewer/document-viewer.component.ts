...
import { saveAs } from 'file-saver'; // EDISSYUM - NCH01 Fix pour le téléchargement des fichiers trop lourds sur Firefox
...
export class DocumentViewerComponent implements OnInit, OnDestroy {
    ...
    @Input() realUserId: number = null; // EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier si le typist est égal à l'utilisateur connecté
    ...
    loadFileFromBase64() {
        this.loading = true;
        this.file = {
            name: 'mem', // EDISSYUM - NCH01 Changement du nom d'application | Remplacer maarch par mem
            format: 'pdf',
            type: 'application/pdf',
            contentMode: 'base64',
            content: this.base64,
    ...
    downloadConvertedFile() {
        ...
        if (this.file.contentMode === 'base64') {
            ...
            // EDISSYUM - NCH01 Fix pour le téléchargement des fichiers trop lourds sur Firefox
            ...
            this.http.get(this.file.content).pipe(
                tap((data: any) => {
                    // EDISSYUM - NCH01 Fix pour permettre l'affichage d'un document si le mimetype est à octet-stream
                    if (this.file.content.includes('signedVersion=true')) {
                        const tmpFileName = data.filename;
                        if (tmpFileName.split('.').pop() === 'pdf' && data.mimeType === 'application/octet-stream') {
                    ...
                    // EDISSYUM - NCH01 Fix pour le téléchargement des fichiers trop lourds sur Firefox | Commenter les lignes ci-dessous
                    const blob = b64toBlob(data.encodedDocument, data.mimeType);
                    const mbSize = blob.size / 1024 / 1024;
            ...
            this.requestWithLoader(`../rest/attachments/${resId}/content?mode=base64`).subscribe(
                (data: any) => {
                    if (data.encodedDocument) {
                        ...
                        this.loading = false;
                        this.noFile = false;
                        // EDISSYUM - NCH01 Fix manque du fingerprint lors de la récupération d'une PJ non non convertible
                        this.allowedExtensions.forEach((ext: any) => {
                            if (ext.extension === '.' + this.file.format && !ext.canConvert) {
                                this.noConvertedFound = true;
                            }
                        });
                        // END EDISSYUM - NCH01
        ...
        return new Promise((resolve) => {
            this.http.get(`../rest/resources/${this.resId}/versionsInformations`).pipe(
                ...
                tap((data: any) => {
                    ...
                    const allowedExtensions = ['doc', 'docx', 'odt', 'dot', 'xlsx', 'xls', 'ods'];
                    if (data['information']['category_id'] !== 'incoming' && this.realUserId && allowedExtensions.includes(data['information']['format']) &&
                        ((data['information']['typist'] && data['information']['typist'].toString() === this.realUserId.toString()) ||
                            (data['information']['dest_user'] && data['information']['dest_user'].toString() === this.realUserId.toString()))) {
                        this.editMode = true;
                    }
                    // END EDISSYUM - NCH01
