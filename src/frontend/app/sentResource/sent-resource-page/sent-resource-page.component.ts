...
export class SentResourcePageComponent implements OnInit {
    ...
    isDisabled: boolean = false; // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
    ...
    async ngOnInit(): Promise<void> {
        this.loading = false;
        // EDISSYUM - NCH01 Fix chargement de l'interface de mail
        setTimeout(() => {
            this.appMailEditor.loading = false;
        }, 1000);
        // END EDISSYUM - NCH01
    }
    ...
    async onSubmit() {
        ...
        if (this.data.emailId === null) {
            if (!this.appMailEditor.isAllEmailRightFormat()) {
                this.isDisabled = false; // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
