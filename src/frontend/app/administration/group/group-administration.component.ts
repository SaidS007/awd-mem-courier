...
export class GroupAdministrationComponent implements OnInit {
    ...
    authorizedStatusUserParams: any[] = [];
    ...
    openParams(id: string) {
        ...
        if (!this.sidenavRight.opened) {
            ...
            } else if (id === 'update_status_mail') {
                this.http.get('../rest/statuses').pipe(
                    map((data: any) => {
                        data.statuses = data.statuses.map((status: any) => ({
                            id: status.id,
                            label: status.label_status + ' (' + status.id + ')',
                        }));
                        return data;
                    }),
                    tap((data: any) => {
                        this.authorizedStatusUserParams = data.statuses;
                    }),
                    exhaustMap(() => this.http.get(`../rest/groups/${this.group.id}/privileges/${this.panelMode}/parameters?parameter=status`)),
                    tap((data: any) => {
                        const allowedStatus: any[] = data;
                        this.authorizedStatusUserParams.forEach(status => {
                            status.checked = allowedStatus.indexOf(status.id) > -1;
                        });
                    }),
                    finalize(() => this.paramsLoading = false),
                    catchError((err: any) => {
    ...
    updatePrivilegeParams(paramList: any) {
        ...
        } else if (this.panelMode === 'update_status_mail') {
            obj = {
                status: paramList.map((param: any) => param.value)
            };
        }
