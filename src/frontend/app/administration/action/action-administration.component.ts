...
import { FormControl } from '@angular/forms'; // EDISSYUM - NCH01 Ajout d'une option pour autoriser ou non la validation du viseur ou du signataire courant avant envoi au parapheur externe | ajout formControl
...
export class ActionAdministrationComponent implements OnInit {
    ...
    // EDISSYUM - NCH01 Ajout d'une option pour autoriser ou non la validation du viseur ou du signataire courant avant envoi au parapheur externe
    visaValidation = new FormControl();
    visaChoice: Array<any> = [
        {
            'id': true,
            'label': this.translate.instant('lang.yes')
        },
        {
            'id': false,
            ...
            if (typeof params['id'] === 'undefined') {
                ...
                this.http.get('../rest/actions/' + params['id'])
                    .subscribe(async (data: any) => {
                        ...
                        // EDISSYUM - NCH01 Fix pour les actions de redirection qui n'ont pas de paramètres par défaut
                        if (this.action.parameters.length === 0 && this.action.actionPageId === 'redirect') {
                        ...
                        this.visaValidation.setValue(this.action.parameters.visaValidation); // EDISSYUM - NCH01 Ajout d'une option pour autoriser ou non la validation du viseur ou du signataire courant avant envoi au parapheur externe
                        this.statuses = data.statuses.map((status: any) => ({
                            id: status.id,
                            label: status.label_status
                        }));
                        ...
                        } else if (this.action.actionPageId === 'close_mail' || this.action.actionPageId === 'send_to_visa') { // EDISSYUM - NCH01 Rajout checkEmptyFields pour l'action système Envoyer pour visa | rajout de && action.actionPageId === 'send_to_visa'
    ...
    getCustomFields() {
        ...
        return new Promise((resolve) => {
            // EDISSYUM - NCH01 Rajout checkEmptyFields pour l'action système Envoyer pour visa | Ajout send_to_visa
            if (['confirm_status', 'close_mail', 'send_to_visa'].includes(this.action.actionPageId)) {
                this.http.get('../rest/customFields').pipe(
                    tap((data: any) => {
                        ...
                        } else if ((this.action.actionPageId === 'close_mail' || this.action.actionPageId === 'send_to_visa') && this.functions.empty(this.availableCustomFields)) { //EDISSYUM - NCH01 Rajout checkEmptyFields pour l'action système Envoyer pour visa | Modifier this.action.actionPageId === 'close_mail' par (this.action.actionPageId === 'close_mail' || this.action.actionPageId === 'send_to_visa')
    ...
    onSubmit() {
        ...
        } else if (this.action.actionPageId === 'close_mail' || this.action.actionPageId === 'send_to_visa') { // EDISSYUM - NCH01 Rajout checkEmptyFields pour l'action système Envoyer pour visa | Ajout de || this.action.actionPageId === 'send_to_visa'
            this.action.parameters = { requiredFields: this.selectedFieldsId };
            // EDISSYUM - NCH01 Rajout checkEmptyFields pour l'action système Envoyer pour visa
            if (this.action.actionPageId === 'send_to_visa') {
                this.action.parameters = { successStatus: this.successStatus, errorStatus: this.errorStatus, requiredFields: this.selectedFieldsId};
            }
            // END EDISSYUM - NCH01
        } else if (this.action.actionPageId === 'create_acknowledgement_receipt') {
        ...
        // EDISSYUM - NCH01 Ajout d'une option pour autoriser ou non la validation du viseur ou du signataire courant avant envoi au parapheur externe
        if (this.action.actionPageId === 'sendToExternalSignatureBook') {
            this.action.parameters['visaValidation'] = this.visaValidation.value;
        }
        // END EDISSYUM - NCH01
