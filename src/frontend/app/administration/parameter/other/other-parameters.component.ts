...
export class OtherParametersComponent implements OnInit {
    ...
    enableDefaultActif : boolean = false;
    msloading: boolean = false;
    mshasError: boolean = false;
    msHasErrorEmail: boolean = false;
    msLoadingEmail: boolean = false;
    msArchivalError: string  = '';
    msResult: string  = '';
    mailSecureEnabled = [];
    mailSecureConf: any = {
        pastell: {
    ...
    async ngOnInit() {
        ...
        // EDISSYUM - ASY01 - Implémentation envoie de Mail sécurisé via Pastell
        await this.getMailSecureConfiguration();
        Object.keys(this.mailSecureConf).forEach(editorId => {
            Object.keys(this.mailSecureConf[editorId]).forEach((elementId: any) => {
                this.mailSecureConf[editorId][elementId].valueChanges
                    .pipe(
                        debounceTime(1000),
                        filter(() => this.mailSecureConf[editorId][elementId].valid),
                        tap(() => {
                            this.saveMailSecure();
                        }),
                    ).subscribe();
    ...
    // EDISSYUM - ASY01 - Implémentation envoie de Mail sécurisé via Pastell
    saveMailSecure() {
        this.http.put('../rest/configurations/admin_send_secure_email', this.formatMailSecure()).pipe(
            tap(() => {
                this.notify.success(this.translate.instant('lang.dataUpdated'));
            }),
            catchError((err: any) => {
    ...
    formatMailSecure() {
        const obj: any = {};
        this.mailSecureEnabled.forEach((id: any) => {
            if (this.mailSecureEnabled.indexOf(id) > -1) {
                obj[id] = {};
                Object.keys(this.mailSecureConf[id]).forEach(elemId => {
                    obj[id][elemId] = this.mailSecureConf[id][elemId].value;
                });
            }
        });
        return obj;
    ...
    checkPastellConnection() {
        ...
        this.http.get('../rest/parameters/pastell/check').pipe(
            tap(() => {
                ...
                this.msHasErrorEmail = false;
                this.msResult = this.translate.instant('lang.pastellConnexionSuccess');
            }),
            catchError((err: any) => {
                this.mshasError = true;
                this.msloading = false;
                this.msArchivalError = err.error.error;
                this.msHasErrorEmail = true;

                this.msResult = this.translate.instant('lang.pastellConnexionFailed');
                return of(false);
            })
        ).subscribe();
    }

    changePastellByActif() {
        if (this.mailSecureConf['pastell']['enabled'].value) {
            this.enableDefaultActif = true;
        } else {
            this.mailSecureConf['pastell']['byDefault'].setValue(false);
            this.enableDefaultActif = false;
        }
    }

    changeNextcloudByDefault() {
        if (this.attachmentsHostsConf['nextcloud']['byDefault'].value) {
            this.mailSecureConf['pastell']['byDefault'].setValue(false);
        }
    }

    changePastellByDefault() {
        if (this.mailSecureConf['pastell']['byDefault'].value) {
            this.attachmentsHostsConf['nextcloud']['byDefault'].setValue(false);
        }
    }
    // END EDISSYUM - ASY01
