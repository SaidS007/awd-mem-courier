...
export class UsersAdministrationRedirectModalComponent implements OnInit {
    ...
    loading: boolean = true; // EDISSYUM - NCH01 Correction de l'apparition de la modale de suspension d'utilisateur
            ...
            this.http.get(`../rest/users/${this.data.user.id}/isDeletable`).pipe(
                tap((response: any) => {
                    if (response?.errors) {
                        ...
                        this.loading = false; // EDISSYUM - NCH01 Correction de l'apparition de la modale de suspension d'utilisateur
                    } else {
    ...
    updateVisaWorkflow() {
        ...
        return new Promise((resolve) => {
            this.http.put('../rest/circuits/visaCircuit', {
                resources: this.userVisaWorkflowResources,
                updateUser : true // EDISSYUM - LBE01 Correctif erreur désactivation utilisateur
            }).pipe(
                tap((data: any) => {
                    if (data?.errors) {
                        this.notify.error(data.errors);
