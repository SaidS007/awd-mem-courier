import { Component, HostListener, Input, OnDestroy, OnInit } from '@angular/core'; // EDISSYUM - NCH01 Correction de la gestion de l'overflow du menu TinyMCE | Ajout de HostListener
...
export class MailSignaturesAdministrationComponent implements OnInit, OnDestroy {
    ...
    // EDISSYUM - NCH01 Correction de la gestion de l'overflow du menu TinyMCE
    @HostListener('document:click', ['$event'])
    onScreenClick(event: MouseEvent) {
        const clickedElement = event.target as HTMLElement;
        const toxToolbarOverflow = document.getElementsByClassName('tox-toolbar__overflow');
        if (toxToolbarOverflow[0] && !toxToolbarOverflow[0].contains(clickedElement)) {
            const toxToolbar = document.getElementsByClassName('tox-toolbar__primary');
            if (toxToolbar) {
                const toxToolbarChildrens = toxToolbar[0].children;
                const lastToxToolbarChildrens = toxToolbarChildrens[toxToolbarChildrens.length - 1];
                const lastToxToolbarChildrensButton = lastToxToolbarChildrens.getElementsByTagName("button");
                lastToxToolbarChildrensButton[0].id = 'toolbarElementToClick';
                document.getElementById('toolbarElementToClick').click();
            }
        }
    }
    // END EDISSYUM - NCH01
