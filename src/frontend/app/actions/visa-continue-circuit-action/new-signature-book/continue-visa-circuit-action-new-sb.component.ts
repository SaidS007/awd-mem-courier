...
export class ContinueVisaCircuitActionNewSbComponent implements OnInit, OnDestroy {
                ...
                .pipe(
                    tap((data: any) => {
                        ...
                        if (!this.functions.empty(data.resourcesInformations.error)) {
                            ...
                            const ignoredResources: number[] = this.resourcesErrors.map((item: any) => item.res_id);
                            this.data.resource.docsToSign = this.data.resource.docsToSign.filter((attachment: Attachment) => ignoredResources.indexOf(attachment.resIdMaster) === -1);
                        }
                        ...
                        if (!this.functions.empty(data.visaWorkflow)) {
                            this.setVisaWorkflow(data);
                        }

                        if (this.data.resource.docsToSign.length > 0 && (this.data.resource.docsToSign as Attachment[]).every((attachment: Attachment) => this.functions.empty(attachment.visaWorkflow))) {
    ...
    executeAction(realResSelected: number[], objToSend: ContinueVisaCircuitObjectInterface = null): void {
    ...
    atLeastOneDocumentHasNoStamp(): boolean {
        if (this.data.resource.length > 0) {
    ...
    atLeastOneDocumentHasDigitalSignature(): boolean {
        if (this.data.resource.docsToSign.length > 0) {
            return (this.data.resource.docsToSign as Attachment[]).some((resource: Attachment) => resource.hasDigitalSignature === true);
        }
        return false;
    }
    ...
    formatVisaWorkflow(visaWorkflow: any[]): WorkflowItemsInterface[] {
        return visaWorkflow.map((user: any) => ({
            ...
            mode: user.item_mode,
            signatureMode: user.item_mode === 'sign' ? 'rgs_2stars' : 'stamp',
            signaturePositions: []
        }));
    }

    hasOnlyVisaUsers(): boolean {
        const allItemsVisa = (this.data.resource.docsToSign as Attachment[]).every((attachment: Attachment) => {
    ...
    setVisaWorkflow(data: any): void {
        Object.keys(data.visaWorkflow).forEach((resId: string) => {
            (this.data.resource.docsToSign as Attachment[])
                .filter((attachment: Attachment) => attachment.resIdMaster === +resId)
                .forEach((item: Attachment) => {
                    item.visaWorkflow = this.formatVisaWorkflow(data.visaWorkflow[resId]);
                });
        });
    }
    ...
    isNotValidAction(): boolean {
        return  this.loading || this.isEmptyWorkflow || !this.isValidAction() ||
                ...
                (this.atLeastOneDocumentHasNoStamp() && !this.parameters.digitalCertificate);
    }
