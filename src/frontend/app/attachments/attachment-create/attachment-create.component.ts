...
import { ActivatedRoute } from '@angular/router'; // EDISSYUM - NCH01 Correction publipostage
...
export class AttachmentCreateComponent implements OnInit {
    ...
    constructor(
        private route: ActivatedRoute, // EDISSYUM - NCH01 Correction publipostage
    ...
    loadResource() {
        return new Promise((resolve) => {
            this.http.get(`../rest/resources/${this.data.resIdMaster}?light=true`).pipe(
                tap(async (data: any) => {
                    let contact: any = '';
                    if (data.categoryId === 'outgoing' || this.route.snapshot.queryParamMap.get('isMailing')) { // EDISSYUM - NCH01 Correction publipostage | Ajout || this.route.snapshot.queryParamMap.get('isMailing')
                    ...
                    // EDISSYUM - NCH01 Fix taille de l'objet d'une PJ
                    if (data.subject) {
                        data.subject = data.subject.substring(0, 255);
                    }
                    // END EDISSYUM - NCH01
    ...
    setDatasViewer(ev: any, i: number) {
        ...
        if (ev === 'uploadFile') {
            ...
            // EDISSYUM - NCH01 Fix pour l'insertion de plusieurs PJ en simultanés
            this.appDocumentViewer.toArray()[i].getFile().pipe(
                take(1),
                tap((data) => {
                    this.attachments[i].encodedFile.setValue(data.content);
                    this.attachments[i].format.setValue(data.format);
                }),
            ).subscribe();
            // END EDISSYUM - NCH01
        }
