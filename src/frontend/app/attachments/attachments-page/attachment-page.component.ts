...
export class AttachmentPageComponent implements OnInit {
    ...
    getAttachmentValues(newAttachment: boolean = false, mode: string) {
        ...
        // EDISSYUM - NCH01 Fix typist des nouvelles version de PJ
        attachmentValues['typist'] = this.headerService.user.id;
        attachmentValues['typistLabel'] = this.headerService.user.firstname + ' ' + this.headerService.user.lastname;
        // END EDISSYUM - NCH01
    ...
    checkVersion(event: any) {
        const version = event.tab._implicitContent['_declarationLView'][0].id;
        let resId = this.attachment['resIdMaster'].value;
        if (version > 0) {
            this.versions.forEach((item: any) => {
                if (item.relation.toString() === version.toString()) {
                    resId = item.resId;
                }
            });

            this.http.get(`../rest/attachments/${resId}?allowObs=true`).pipe(
                tap((data: any) => {
                    this.attachment['typist'].setValue(data.typist);
                    this.attachment['typistLabel'].setValue(data.typistLabel);
                    this.attachment['creationDate'].setValue(data.creationDate);
                    this.attachment['relation'].setValue(data.relation);
                }),
                catchError((err: any) => {
                    this.notify.handleSoftErrors(err);
                    this.dialogRef.close('');
