...
export class SignatureBookOldComponent implements OnInit, OnDestroy {
        ...
        this.route.params.subscribe(params => {
            ...
            this.http.get('../rest/signatureBook/users/' + this.userId + '/groups/' + this.groupId + '/baskets/' + this.basketId + '/resources/' + this.resId)
                .subscribe((data: any) => {
                    ...
                    // EDISSYUM - NCH01 Retour à la banette après un F5 dans le parapheur interne
                    if (!this.signatureBook.resList) {
                        this.backToBasket();
                    } else {
                        this.signatureBook.resListIndex = this.signatureBook.resList.map((e: any) => e.res_id).indexOf(this.resId);
                    }
                    // END EDISSYUM - NCH01
                    // this.signatureBook.resListIndex = this.signatureBook.resList.map((e: any) => e.res_id).indexOf(this.resId);// EDISSYUM - NCH01 Retour à la banette après un F5 dans le parapheur interne | Commenter cette ligne
        ...
        if (!this.loadingSign && this.signatureBook.canSign) {
            ...
            this.http.put(route, { 'signatureId': signature.id })
                .subscribe((data: any) => {
                    ...
                    this.ngOnInit(); // EDISSYUM - NCH01 Recharger la liste des actions après la signature d'un document
                }, (error: any) => {
    ...
    unSignMainDocument(attachment: any) {
        this.http.put(`../rest/resources/${attachment.res_id}/unsign`, {}).pipe(
            tap(() => {
                // this.appDocumentViewer.loadRessource(attachment.res_id, 'mainDocument'); // EDISSYUM - NCH01 Fix pour éviter une erreur à la désignature d'un document | Commenter cette ligne
                // EDISSYUM - NCH01 Fix pour éviter une erreur à la désignature d'un document
                if (this.appDocumentViewer !== undefined) {
                    this.appDocumentViewer.loadRessource(attachment.res_id, 'mainDocument');
                }
                // END EDISSYUM - NCH01
    ...
    unSignAttachment(attachment: any) {
        this.http.put('../rest/attachments/' + attachment.res_id + '/unsign', {}).pipe(
            tap(() => {
                // this.appDocumentViewer.loadRessource(attachment.res_id, 'attachment'); // EDISSYUM - LBE01 - Ticket 14845 - Fix pour éviter une erreur à la désignature d'une PJ
                // EDISSYUM - LBE01 - Ticket 14845 - Fix pour éviter une erreur à la désignature d'une PJ
                if (this.appDocumentViewer !== undefined) {
