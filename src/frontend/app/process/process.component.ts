import { Component, OnInit, ViewChild, ViewContainerRef, TemplateRef, OnDestroy, inject } from '@angular/core';
...
import { AiMergeContactComponent } from "@appRoot/administration/contact/ai-merge-contact/ai-merge-contact.component";
...
export class ProcessComponent implements OnInit, OnDestroy {
    ...
    sereniaService = inject(SereniaChatbotService); // EDISSYUM - NCH01 Implémentation SerenIA
    ...
    constructor(
        ...
        this.route.params.subscribe(params => {
            ...
            if (typeof params['detailResId'] !== 'undefined') {
                ...
                this.sereniaService.resId = params['detailResId']; // EDISSYUM - NCH01 Implémentation SerenIA
            } else {
                // EDISSYUM - NCH01 Implémentation SerenIA
                if (params['resId'] !== undefined) {
                    this.sereniaService.resId = params['resId'];
                }
                // END EDISSYUM - NCH01
                this.initProcessPage(params);
            }

            this.sereniaService.actionEvent = this.handleSereniaAction.bind(this); // EDISSYUM - NCH01 Implémentation SerenIA
    ...
    async initDetailPage(params: any) {
        this._activatedRoute.queryParamMap.subscribe((paramMap: ParamMap) => {
            ...
            if (!this.functions.empty(paramMap.get('fromSearch'))) {
                this.isFromSearch = paramMap.get('fromSearch') === 'true';
            }
            // END EDISSYUM - EME01
        });
    ...
    async ngOnDestroy() {
        ...
        // EDISSYUM - NCH01 Implémentation SerenIA
        this.sereniaService.resId = '';
        this.sereniaService.actionsList = [];
        this.sereniaService.showDialog = false;
        this.sereniaService.keepSereniaIcon = false;
        this.sereniaService.newNotification = false;
        // END EDISSYUM - NCH01
    }
    ...
    canLaunchAction() {
        ...
        if (currentActions.length > 0 && currentActions.find((action: any) => action.id === this.selectedAction.id) !== undefined) {
            ...
            // EDISSYUM - NCH01 Ajout d'un paramètre pour envoyer le document principal lors de l'envoi au visa
            let sendersLength = 0;
            let recipientsLength = 0
            if (this.currentResourceInformations.senders) {
                sendersLength = this.currentResourceInformations.senders.length;
            }
            if (this.currentResourceInformations.recipients) {
                recipientsLength = this.currentResourceInformations.recipients.length;
            }

            if (sendersLength > 1 || recipientsLength > 1 || this.isMailing || this.selectedAction.component == 'sendExternalSignatoryBookAction') {
                this.http.put('../rest/resourcesList/integrations', { resources: [this.currentResourceInformations.resId], integrations: { inSignatureBook: false } }).pipe(
                    tap(() => {
                        this.currentResourceInformations.integrations.inSignatureBook = false;
                    }),
                    catchError((err: any) => {
                        this.notify.handleSoftErrors(err);
                        return of(false);
            ...
            if ((sendersLength <= 1 && recipientsLength <= 1) && !this.isMailing && this.indexingForm.indexingModelClone.sendMainDocParaph === true
                && !this.currentResourceInformations.integrations.inSignatureBook) {
                this.http.put('../rest/resourcesList/integrations', { resources: [this.currentResourceInformations.resId], integrations: { inSignatureBook: true } }).pipe(
                    tap(() => {
                        this.currentResourceInformations.integrations.inSignatureBook = true;
                    }),
                    catchError((err: any) => {
                        this.notify.handleSoftErrors(err);
                        return of(false);
    ...
    goToResource(event: 'next' | 'previous' = 'next') {
        this.sessionStorage.save('currentTool', this.currentTool);

        // EDISSYUM - NCH01 Implémentation SerenIA
        this.sereniaService.actionsList = [];
        this.sereniaService.messagesList = [];
        this.sereniaService.showDialog = false;
        this.sereniaService.newNotification = false;
        this.sereniaService.keepSereniaIcon = false;
        if (this.indexingForm) {
            this.indexingForm.appContactAutocomplete.valuesToDisplay['contact'] = {};
        }
        // END EDISSYUM - NCH01
    ...
    disableSubmit() {
        let _return = false;
        if (this.indexingForm?.arrFormControl['senders']) {
            this.indexingForm?.arrFormControl['senders']?.value.forEach((sender: any) => {
                if (this.indexingForm.appContactAutocomplete.valuesToDisplay['contact'][sender['id']]) {
                    const sender_info = this.indexingForm.appContactAutocomplete.valuesToDisplay['contact'][sender['id']];
                    if ('status' in sender_info && (sender_info['status'] == 'TMP' ||
                        (sender_info['externalId']['ia_tmp_contact_id']) && this.currentResourceInformations['externalId'] &&
                        this.currentResourceInformations['externalId']['ia_tmp_contact_id'] &&
                        this.currentResourceInformations['externalId']['ia_tmp_contact_id'] == sender_info['externalId']['ia_tmp_contact_id'])) {
                        _return = true;

                        this.sereniaService.newNotification = true;
                        this.sereniaService.keepSereniaIcon = true;
                        if (this.sereniaService.actionsList.length == 0) {
                            if (!this.sereniaService.messagesList.includes(this.translate.instant('lang.serenia_detect_temporary_contact'))) {
                                this.sereniaService.messagesList.push(this.translate.instant('lang.serenia_detect_temporary_contact'));
                            }
                            this.sereniaService.actionsList.push({
                                'id': sender_info['status'] == 'TMP' ? 'modifyContact' : 'mergeContact',
                                'data': { "contact_id": sender["id"], "ia_tmp_contact_id": sender_info['externalId']['ia_tmp_contact_id'] },
                                'color': 'primary',
                                'label': sender_info['status'] == 'TMP' ? 'lang.serenia_validate_contact' : 'lang.serenia_merge_contact'
                            });
                            this.sereniaService.actionsList.push({
                                'id': 'deleteContact',
                                'data': { "contact_id": sender["id"], "ia_tmp_contact_id": sender_info['externalId']['ia_tmp_contact_id'] },
                                'color': 'warn',
                                'label': 'lang.serenia_delete_contact'
                            });
                        }
                    }
                }
    ...
    // EDISSYUM - NCH01 Implémentation SerenIA
    handleSereniaAction(action: any) {
        const contact = { id: action.data['contact_id'], type: "contact" }
        if (action.id == 'modifyContact') {
            this.indexingForm.appContactAutocomplete.openContact(contact, true);
        } else if (action.id == 'mergeContact') {
            const externalId = { ia_tmp_contact_id: action.data['ia_tmp_contact_id'] };
            this.mergeContact(contact, externalId);
        } else if (action.id == 'deleteContact') {
            this.indexingForm.appContactAutocomplete.removeItem(null, contact);
        }
    }
    // END EDISSYUM - NCH01

    // EDISSYUM - NCH01 Module OCForMEM NextGen
    mergeContact(masterContact: any, masterContactExternalId: any) {
        if (!this.privilegeService.hasCurrentUserPrivilege('update_contacts')) {
            this.notify.error(this.translate.instant('lang.noPrivileges'));
            return;
        }

        if (masterContact && masterContact['id']) {
            const slaveContact = { 'id': masterContactExternalId["ia_tmp_contact_id"], 'type': 'contact' };
            const contactToMerge = [
                masterContact,
                slaveContact
            ];
            const dialogRef = this.dialog.open(AiMergeContactComponent, {
                panelClass: 'maarch-modal',
                data: { duplicate: contactToMerge, currentResourceInformations: this.currentResourceInformations },
            });
            dialogRef.afterClosed().pipe(
                tap((data: any) => {
                    if (data) {
                        this.sereniaService.actionsList = [];
                        this.indexingForm.appContactAutocomplete.controlAutocomplete.setValue(data);
                        this.indexingForm.appContactAutocomplete.ngOnInit();
                    }
                }),
                catchError((err: any) => {
                    this.notify.handleErrors(err);
