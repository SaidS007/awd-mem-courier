...
import { ContactResourceModalComponent } from '@appRoot/contact/contact-resource/modal/contact-resource-modal.component'; // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers
...
export class FolderDocumentListComponent implements OnInit, OnDestroy {
    ...
    refreshBadgeNotes(nb: number) {
        this.currentResource.countNotes = parseInt(String(nb)); // EDISSYUM - NCH01 Fix affichage des pastilles quand le compteur est à 0
    }
    ...
    refreshBadgeAttachments(nb: number) {
        this.currentResource.countAttachments = parseInt(String(nb)); // EDISSYUM - NCH01 Fix affichage des pastilles quand le compteur est à 0
    }

    refreshBadgeSentResource(nb: number) {
        this.currentResource.countSentResources = parseInt(String(nb)); // EDISSYUM - NCH01 Fix affichage des pastilles quand le compteur est à 0
    }
                ...
                if ((key.displayValue == null || key.displayValue === '') && ['getCreationAndProcessLimitDates', 'getParallelOpinionsNumber'].indexOf(key.value) === -1) {
                    ...
                    key.displayValue.forEach((visa: any, keyVis: number) => {
                        ...
                        if (visa.date === '') {
                            ...
                            content = '<span style="color:#006841" style=""><i class="fa fa-check"></i> <span title="' + this.translate.instant('lang.' + visa.mode + 'User') + '">' + user + '</span></span>';
                        }

                        if (visa.current && keyVis >= 0) {
                            content = '<b style="color:#3A7C00">' + content + '</b>';
                        }
                    ...
                    if (key.displayValue > 0) {
                        key.displayValue = '<b style="color:#3A7C00">' + key.displayValue + '</b> ' + this.translate.instant('lang.opinionsSent');
                    } else {
