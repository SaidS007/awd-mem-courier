...
export class CriteriaToolComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        ...
        if (this.currentParam?.target === 'multiCriteria' && !this.functions.empty(this.currentParam.value)) {
            this.criteriaSearchService.updateListsPropertiesCriteria({multiCriteria: {type: 'autocomplete', values: [this.currentParam.value]}});
        }
        // END EDISSYUM - NCH01
