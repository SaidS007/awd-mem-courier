...
import { MatCheckboxChange } from '@angular/material/checkbox'; // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères
...
export class MailEditorComponent implements OnInit, OnDestroy {
    ...
    checkBoxAttachments: any = new UntypedFormControl();
    hasError: boolean = false;
    errorMessage: string = '';
    nextcloudFieldsFilled: boolean = false;
    sentWithNextcloud: boolean = false;
    attachmentsHostsConf: any = {
        nextcloud: {
            byDefault: new UntypedFormControl(false),
            url: new UntypedFormControl(''),
            username: new UntypedFormControl(''),
            password: new UntypedFormControl(''),
            folderNextcloud : new UntypedFormControl(''),
            urlExpirationDate : new UntypedFormControl(''),
            textAddedAboveURLS : new UntypedFormControl('Pièces Jointes :'),
        }
    };
    ...
    msHasError: boolean   = false;
    mailsecFieldsFilled: boolean   = false;
    mailSecureEnabled = [];
    mailSecureConf: any = {
        pastell: {
            ...
            password: new UntypedFormControl(''),
            entite : new UntypedFormControl(0),
        }
    };
    // END EDISSYUM - ASY01
    ...
    async ngOnInit() {
            ...
            Object.keys(this.emailAttachTool).forEach(element => {
                if (element === 'document') {
                    this.emailAttach[element] = {
                        ...
                        isLinked: false,
                        original: false,
                        mailSec: false, // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell | Ajout mailSec
                        receipt: false // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell | ajout receipt
                    };
                } else {
                    this.emailAttach[element] = [];
                }
        ...
        // EDISSYUM - ASY01 - Implémentation envoie de Mail sécurisé via Pastell
        await this.getPastellConfiguration();
        this.mailsecFieldsFilled = (this.mailSecureConf['pastell'].login.value !== '' &&  this.mailSecureConf['pastell'].password.value !== '' &&  this.mailSecureConf['pastell'].url.value !== '' && this.mailSecureConf['pastell'].entite.value !== '');

        if (this.mailsecFieldsFilled) {
            await this.checkPastellConnection();
        } else {
            this.msHasError = true;
            this.mailSecureConf['pastell']['byDefault'].setValue(false);
        }
        // END EDISSYUM - ASY01
                ...
                tap((data: any) => {
                    if (data) {
                        Object.keys(data).forEach(confId => {
                            ...
                            if (this.attachmentsHostsConf['nextcloud']['byDefault']['value']) {
                ...
                tap(async (data: any) => {
                    ...
                    Object.keys(data.document).forEach(element => {
                        if ([
                            ...
                            'original',
                            'resource',
                            'mailSec', // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell | Ajout mailSec
                            'receipt' // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell | Ajout receipt
                        ].indexOf(element) === -1) {
    ...
    initMce() {
        tinymce.init({
            ...
            height: '400',
            suffix: '.min',
            browser_spellcheck: true, // EDISSYUM - NCH01 Ajout du correcteur orthographique lors de la rédaction d'un mail
            language: this.translate.instant('lang.langISO').replace('-', '_'),
            // language_url: `../dist/tinymce-i18n/langs/${this.translate.instant('lang.langISO').replace('-', '_')}.js`, // EDISSYUM - LBE01 Correction traductions TinyMCE | commenter cette ligne
            language_url: `../node_modules/tinymce-i18n/langs${tinymce.majorVersion >= 5 ? tinymce.majorVersion:''}/${this.translate.instant('lang.langISO').replace('-', '_')}.js`, // EDISSYUM - LBE01 Correction traductions TinyMCE | dossier "langs" de la version utilisée
            menubar: false,
            statusbar: false,
            plugins: [
                'autolink'
            ...
            this.http.get(`../rest/contactsGroups/${item.id}/correspondents?limit=none`).pipe(
                    ...
                    if (data.length !== tmpData.length) {
                        tmpData.correspondents.forEach((contact: any, cpt: number) => {
                            if (contact.type !== 'contactGroup') {
                                data.forEach((contactBis: any) => {
                                    if (contactBis.id === contact.id) {
                ...
                tap((data: any) => {
                    if (this.functions.empty(data)) {
                        ...
                        // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre
                        this[type] = this[type].filter((item2: any, index: number) => {
                            return this[type].findIndex((item3: any) => item3.email === item2.email) === index;
                        });
                        // END EDISSYUM - NCH01
                    }
                }),
    ...
    // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre
    setContactRecursively(contacts: any, type: any) {
    ...
    formatEmail() {
        ...
        // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell
        objAttach['mailSec'] = this.mailSecureConf['pastell'].byDefault.value;
        objAttach['receipt'] = this.receivedReceipt;
        // END EDISSYUM - ASY01
        ...
        return {
            ...
            pjLinkChecked : this.checkBoxAttachments.value, // EDISSYUM - AMO01 Rajout d'une option pour envoyer les PJ via liens épémères
            mailSecChecked : this.mailSecureConf['pastell'].byDefault.value, // EDISSYUM - ASY01 - Implémentation envoie de Mail sécurisé via Pastell
    ...
    openDocument(type: string, document: any): void {
        if (!this.functions.empty(document.convertedDocument) || !this.functions.empty(document.id)) { // EDISSYUM - NCH01 Fix pour la prévisualisation des PJ d'un e-mail | Ajout de || !this.functions.empty(document.id)
            if (type === 'attachments') {
    ...
    removeAttachMail(index: number, type: string) {
        if (type === 'document') {
            ...
            this.emailAttach.document.mailSec = false; // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell | ajout mailSec
            this.emailAttach.document.receipt = false; // EDISSYUM - ASY01 Implémentation envoie de Mail sécurisé via Pastell | ajout receipt
    ...
    // EDISSYUM - ASY01 - Implémentation envoie de Mail sécurisé via Pastell
    checkPastellConnection() {
        this.http.get('../rest/parameters/pastell/check').pipe(
            tap(() => {
                this.msHasError = false;
            }),
            catchError(() => {
                this.msHasError = true;
                this.mailSecureConf['pastell']['byDefault'].setValue(false);
    ...
    getPastellConfiguration() {
        return new Promise((resolve) => {
            this.http.get('../rest/configurations/admin_send_secure_email').pipe(
                map((data: any) => data.configuration.value),
                tap((data: any) => {
                    if (data) {
                        Object.keys(data).forEach(confId => {
                            this.mailSecureEnabled.push(confId);
                            Object.keys(data[confId]).forEach(itemId => {
                                if (!this.functions.empty(this.mailSecureConf[confId][itemId])) {
                                    this.mailSecureConf[confId][itemId].setValue(data[confId][itemId]);
                                }
                            });
                        });
                    }
    ...
    changeAccuseLecture() {
        if (this.mailSecureConf['pastell']['byDefault'].value) {
