...
export class PluginAutocompleteComponent implements OnInit {
    ...
    @Input() targetSecondarySearchKey: string; // EDISSYUM - AMO01 Chercher un statut de courrier par son id
    ...
    private _filter(value: string): string[] {
        if (typeof value === 'string') {
            ...
            // return this.datas.filter((option: any) => this.latinisePipe.transform(option[this.targetSearchKey].toLowerCase()).includes(filterValue)); // EDISSYUM - AMO01 Chercher un statut de courrier par son id - Commenter cette ligne
            // EDISSYUM - AMO01 Chercher un statut de courrier par son id
            return this.datas.filter((option: any) => {
                const primaryMatch = this.latinisePipe.transform(option[this.targetSearchKey].toLowerCase()).includes(filterValue);
                const secondaryMatch = this.targetSecondarySearchKey ? this.latinisePipe.transform(option[this.targetSecondarySearchKey].toLowerCase()).includes(filterValue) : false;
