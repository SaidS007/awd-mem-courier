...
export class AuthService {
    ...
    authUri: string = '';
    authNonce: string = ''; // EDISSYUM - NCH01 Ajout d'une méthode de connexion OIDC
    ...
    async logout(cleanUrl: boolean = true, forcePageLogin: boolean = false, history: boolean = false) {
        ...
        if ([
            'cas',
            'keycloak',
            'sso', // EDISSYUM - NCH01 Amélioration de la deconnexion SSO | Ajout 'sso' dans la condition
            'oidc' // EDISSYUM - NCH01 Ajout d'une méthode de connexion OIDC | Ajout de oidc
        ].indexOf(this.authMode) > -1 && !forcePageLogin) {
    ...
    SsoLogout(cleanUrl: boolean = true) {
        this.http
            ...
            .pipe(
                tap(async (data: any) => {
                    // EDISSYUM - NCH01 Ajout d'une méthode de connexion OIDC | Commenter ces lignes
                    // this.redirectAfterLogout(cleanUrl);
                    ...
                    if (data.logoutUrl) {
                        ...
                        if (['process'].indexOf(this.router.url.split('/')[1]) > -1) {
                            this.router.navigate(['/login'], { queryParams: { stopOidc: true } });
                        } else {
                            //await this.router.navigate(['/login']);
                            await this.router.navigate(['/login'], { queryParams: { stopOidc: true } });
