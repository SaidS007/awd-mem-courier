...
if ($synchronizeUsers) {
    $maarchUsers = \User\models\UserModel::get(['select' => ['id', 'LOWER(user_id) as user_id', 'firstname', 'lastname', 'phone', 'mail', 'preferences', 'status', 'mode', 'external_id']]); // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
...
function getUsersEntries($xmlfile)
{
    ...
    // EDISSYUM - OBR01 Ajout de la pagination lors de la récupération des utilisateurs LDAP
    $cookie = '';
    $ldapEntries = [];
    $ldapUserCpt = 0;

    $paginationPageSize = 100;
    if (!empty($xmlfile->paginationPageSize)) {
        $paginationPageSize = $xmlfile->paginationPageSize;;
    }

    $activatePagination = false;
    if (!empty($xmlfile->activatePagination) && (string)$xmlfile->activatePagination == 'true') {
    ...
    if ($activatePagination) {
        do {
            $search = ldap_search(
                $ldap, $dn, ($filters ?? 'cn=*'), [], 0, 0, 0, LDAP_DEREF_NEVER,
                [['oid' => LDAP_CONTROL_PAGEDRESULTS, 'value' => ['size' => $paginationPageSize, 'cookie' => $cookie]]]
            );
            if ($search === false) {
...
function synchronizeUsers(array $ldapUsers, array $maarchUsers)
{
    ...
    foreach ($ldapUsers as $user) {
        ...
        $user['phone'] = trim($user['phone']); // EDISSYUM - PYB01 Suppression des espaces dans les numéros de téléphone pour la synchro LDAP
        if (!empty($maarchUsersLogin[$user['user_id']]) && $maarchUsersLogin[$user['user_id']]['status'] != 'DEL') {
            if ($maarchUsersLogin[$user['']]['status'] == 'SPD') {
            ...
            if ($user['firstname'] != $maarchUsersLogin[$user['user_id']]['firstname']
                ...
                $user['external_id'] = $maarchUsersLogin[$user['user_id']]['external_id'];
                $curlResponse = \SrcCore\models\CurlModel::exec([
            ...
            $user['userId'] = $user['user_id']; // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
...
{
    if (!\Respect\Validation\Validator::stringType()->length(1, 128)->notEmpty()->validate($user['user_id'] ?? null) || !preg_match("/^[\w.@-]*$/", $user['user_id'])) {
...
function userAddEntity($userId, $user)
{
    ...
    if (!empty($entityId)) {
        ...
        if ($curlResponse['code'] != 200) {
            writeLog(['message' => "[ERROR] Add entity ({$entityId}) to user failed : {$curlResponse['response']['errors']}"]);
        }
    } else {
