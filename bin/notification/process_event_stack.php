...
foreach ($events as $event) {
    ...
    // EDISSYUM - NCH01 Fix pour utiliser le filtre des status dans les notifications
    $diffusionParams = empty($notification['diffusion_properties']) ? [] : explode(",", $notification['diffusion_properties']);
    if (
        in_array($notification['diffusion_type'],['dest_user', 'copy_list', 'dest_entity', 'dest_user_visa', 'dest_user_sign'])
        && $event['res_id']
        && $diffusionParams
    ) {
        $resource = ResModel::get(['select' => ['res_id', 'status'], 'where' => ['res_id IN (?)'], 'data' => [$event['res_id']]]);
        if ($resource) {
            if (!in_array($resource[0]['status'], $diffusionParams)) {
                continue;
            }
        }
    }
