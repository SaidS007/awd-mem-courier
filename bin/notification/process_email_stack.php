...
// EDISSYUM - NCH01 Utiliser le dossier custom pour la génération du fichier de lock pour l'envoi des notifications
if (is_dir("{$file['config']['maarchDirectory']}/custom/{$customID}/bin/notification/")) {
    $GLOBALS['lckFile'] = "{$file['config']['maarchDirectory']}/custom/{$customID}/bin/notification/{$customID}process_email_stack.lck";
}
// END EDISSYUM - NCH01
...
foreach ($emails as $key => $email) {
    ...
    if (in_array($configuration['type'], ['smtp', 'mail'])) {
        ...
        $mail_secure = $configuration['secure'];
        ...
        if (!empty($mail_secure)) { // EDISSYUM - NCH01 Ajout d'une possibilité de connexion avec du TLS non sécurisé | Remplacer $configuration['secure'] par $mail_secure
            $phpmailer->SMTPSecure = $mail_secure; // EDISSYUM - NCH01 Ajout d'une possibilité de connexion avec du TLS non sécurisé | Remplacer $configuration['secure'] par $mail_secure
        }
    ...
    // Rajout de la possibilité d'envoyer les PJ dans les notifications | Commenter ce bloc
...
//    }
    ...
    if (!empty($email['attachments'])) {
        ...
        foreach ($attachments as $attachment) {
            if (is_file($attachment)) {
                $prefix = $cpt = '';
                if (str_contains($attachment, 'attachment')) {
                    $prefix = 'PJ_' . $lastDoc . '_';
                    $cpt = $num_pj + 1;
                    $num_pj += 1;
                } else {
                    $num_pj = 0;
                    $lastDoc = str_pad(($num + 1), 4, '0', STR_PAD_LEFT);
                    $cpt = $lastDoc;
                    $num += 1;
                }
                $ext = strrchr($attachment, '.');
