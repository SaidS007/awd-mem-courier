    ...
    foreach ($groups as $group) {
        ...
        if ($notification['diffusion_type'] == 'dest_user') {
            ...
            $tmpUsersOfInstance = ListInstanceModel::get([
                ...
                'where'  => ['difflist_type = ?', 'item_mode IN (?)'], // EDISSYUM - NCH01 Fix pour l'envoi des notifications de la banette des courriers en copie | Remplacer = ? par IN (?)
                'data'   => ['entity_id', ['dest', 'cc']] // EDISSYUM - NCH01 Fix pour l'envoi des notifications de la banette des courriers en copie | Remplacer "'dest'" par ['dest', 'cc']
            ]);
            $usersTmp = $users;

            // EDISSYUM - NCH01 Amélioration des notifications des bannettes | Commenter le code ci-dessous
...
//            });
            ...
            // EDISSYUM - NCH01 Amélioration des notifications des bannettes
            $users = [];
            foreach ($usersTmp as $userTmp) {
                foreach ($tmpUsersOfInstance as $userOrEntity) {
                    ...
                    } else if ($userOrEntity['item_type'] == 'user_id' && $userOrEntity['item_id'] != $userTmp['user_id']) {
                        $userId = UserModel::getById(['id' => $userOrEntity['item_id'], 'select' => ['user_id']]);
                        $usersFromEntity = DatabaseModel::select([
                            'select'    => ['users.user_id', 'users.id'],
                            'table'     => ['entities', 'users_entities', 'users'],
                            'left_join' => ['users_entities.entity_id = entities.entity_id', 'users_entities.user_id = users.id'],
                            'where'     => ['UPPER(entities.entity_id) = ?'],
                            'data'      => [strtoupper($userId['user_id'])]
                        ]);
        ...
        $cpt = 0; // EDISSYUM - NCH01 Fix pour l'envoi d'un grand nombre de notifications
        foreach ($users as $userToNotify) {
            // EDISSYUM - NCH01 Fix pour l'envoi d'un grand nombre de notifications
            $cpt++;
            if ($cpt % 1000 == 0) {
            ...
            $allGroups = \SrcCore\models\DatabaseModel::select([
                ...
                'left_join' => ['usergroups.id = usergroup_content.group_id', 'usergroups.group_id = groupbasket.group_id'],
                'table'     => ['usergroups', 'usergroup_content', 'groupbasket'],
                'where'     => ['user_id = ?', 'basket_id = ?'],
                'data'      => [$userToNotify['id'], $basket['basket_id']]
            ]);
...
$usersId = array_unique($usersId); // EDISSYUM - NCH01 Fix pour l'envoi d'un grand nombre de notifications
...
foreach ($tmpNotifs as $login => $tmpNotif) {
    ...
    foreach ($tmpNotif['baskets'] as $basketId => $basket_list) {
        // EDISSYUM - NCH01 Fix pour l'envoi des notifications si basketId est vide
        if (!$basketId) {
            continue;
        }
        // END EDISSYUM - NCH01
            ...
            if ($attachMode) {
                ...
                foreach ($events as $event) {
                    if ($event['res_id'] != '') {
                        ...
                        // EDISSYUM - NCH01 Rajout de la possibilité d'envoyer les PJ dans les notifications
                        $attachmentsTypeAllowed = ParameterModel::getById(['select' => ['param_value_string'], 'id' => 'notificationAttachmentsTypeAllowed']);
                        if ($attachmentsTypeAllowed) {
                            $attachmentsTypeAllowed = explode(',', $attachmentsTypeAllowed['param_value_string']);
                            $attachmentsTypeAllowed = array_map('trim', $attachmentsTypeAllowed);
                            $attachmentsToAttach = \Attachment\models\AttachmentModel::get([
                                'select' => ['path', 'filename', 'docserver_id'],
                                'where'  => ['res_id_master = ?', 'attachment_type IN (?)', 'status <> ?'],
                                'data'   => [$event['res_id'], $attachmentsTypeAllowed, 'DEL']
                            ]);
                            if ($attachmentsToAttach) {
                                foreach ($attachmentsToAttach as $attachment) {
                                    if (!empty($attachment['docserver_id'])) {
