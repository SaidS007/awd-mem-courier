...
try {
    ...
    if (!in_array($configRemoteSignatoryBook['id'], ['maarchParapheur', 'fastParapheur', 'iParapheur', 'ixbus', 'pastell', 'blueway', 'lexPersona', 'sesile'])) { // EDISSYUM - PYB01 Ajout du connecteur Blueway, Ajout du connecteur Pastell| added blueway, pastell, EDISSYUM - YAA01 Ajout du connecteur lexPersona, Ajout du connecteur sesile
...
$attachments = AttachmentModel::get([
    'select' => ['res_id', '(external_state->>\'signatureBookWorkflow\')::jsonb->>\'fetchDate\' as external_state_fetch_date', 'external_id->>\'signatureBookId\' as external_id', 'format', 'res_id_master', 'title', 'identifier', 'attachment_type', 'recipient_id', 'recipient_type', 'typist', 'origin_id', 'relation'],
...
if ($configRemoteSignatoryBook['id'] == 'ixbus') {
    ...
    $retrievedMails = \ExternalSignatoryBook\controllers\PastellController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'noVersion']); // EDISSYUM - PYB01 Ajout du connecteur Pastell
    // EDISSYUM - PYB01 Ajout du connecteur Pastell | Commenter le code ci-dessous
...
} elseif ($configRemoteSignatoryBook['id'] == 'lexPersona') { // EDISSYUM - YAA01 Ajout du connecteur lexPersona | rajout elseif
    $retrievedMails = \ExternalSignatoryBook\controllers\LexPersonaController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'noVersion']);
} elseif ($configRemoteSignatoryBook['id'] == 'sesile') { // EDISSYUM - YAA01 Ajout du connecteur sesile | rajout elseif
    $retrievedMails = \ExternalSignatoryBook\controllers\SesileController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'noVersion']);
}
...
if (!empty($idsToRetrieve['resLetterbox'])) {
    if ($configRemoteSignatoryBook['id'] == 'maarchParapheur') {
        ...
        $retrievedLetterboxMails = \ExternalSignatoryBook\controllers\PastellController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'resLetterbox']); // EDISSYUM - PYB01 Ajout du connecteur Pastell
        ...
        $retrievedLetterboxMails = \ExternalSignatoryBook\controllers\LexPersonaController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'resLetterbox']);
    } elseif ($configRemoteSignatoryBook['id'] == 'sesile') { // EDISSYUM - YAA01 Ajout du connecteur sesile | rajout elseif
        $retrievedLetterboxMails = \ExternalSignatoryBook\controllers\SesileController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'resLetterbox']);
    }
...
foreach ($retrievedMails['noVersion'] ?? [] as $resId => $value) {
    ...
    if ($value['status'] == 'validated') {
        if (!empty($value['encodedFile'])) {
            ...
            //EDISSYUM - AMI01 Fix Nom du signataire lors de l'envoi au parapheur externe
            $signatory_user_serial_id = \Entity\models\ListInstanceModel::get([
                'select'  => ['item_id'],
            ...
            $return = Bt_createAttachment([
                ...
                'originId'                 => $resId,
                'signatory_user_serial_id' => $value['signatory_user_serial_id'] ?? $signatory_user_serial_id[0]['item_id'] //EDISSYUM - AMI01 Fix Nom du signataire lors de l'envoi au parapheur externe | Changement de "?? null" en "?? $signatory_user_serial_id[0]['item_id']"
            ]);
            if (!empty($return['id'])) {
