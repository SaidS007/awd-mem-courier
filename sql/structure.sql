...
(
    ...
    indexation_parameters jsonb                 NOT NULL DEFAULT '{
        "actions": [],
        "entities": [],
        "keywords": []
    }',
...
(
    ...
    title                    text                                 DEFAULT NULL::text, -- EDISSYUM - NCH01 Fix taille de l'objet d'une PJ | Remplacer character varying(255) par text
...
SELECT *
FROM notes
WHERE unaccent(note_text) ~*
      concat('\m(', array_to_string(array((select unaccent(term) from blacklist)), '|', ''), ')\M');
...
(
    ...
    status              CHARACTER VARYING(256)               DEFAULT 'OK', -- EDISSYUM - NCH01 Module OCForMEM NextGen
...
(
    ...
    exportable  boolean NOT NULL DEFAULT FALSE, -- EDISSYUM ASY01 Ajout de modèle d'exportation PDF/CSV
...
(
    ...
    requested_signature             boolean                         default false,
    signatory                       boolean                         default false,
    old_destination                 VARCHAR, -- EDISSYUM - NCH01 Amélioration de l'historique de la listinstance pour l'IA
    ai_destination                  VARCHAR, -- EDISSYUM - NCH01 Amélioration de l'historique de la listinstance pour l'IA
...
(
    ...
    force_current_user   BOOLEAN DEFAULT FALSE  NOT NULL, -- EDISSYUM - NCH01 Ajout d'un paramètre pour forcer l'utilisateur courant en tant qu'attributaire
    no_diffusion_role    BOOLEAN DEFAULT FALSE  NOT NULL, -- EDISSYUM - NCH01 Ajout d'un paramètre pour ne pas remonter les rôles de diffusion dans l'indexation
    send_main_doc_paraph BOOLEAN DEFAULT FALSE  NOT NULL, -- EDISSYUM - NCH01 Ajout d'un paramètre pour envoyer le document principal lors de l'envoi au visa
    generate_ar          BOOLEAN DEFAULT FALSE  NOT NULL, -- EDISSYUM - NCH01 Rajout d'une option pour pouvoir générer un AR par modèle d'enregistrement
...
CREATE OR REPLACE FUNCTION public.increase_chrono(chrono_seq_name text, chrono_id_name text)
    returns table
            (
                chrono_id bigint
            )
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 0 FROM pg_class where relname = LOWER(chrono_seq_name)) THEN -- EDISSYUM - NCH01 Fix de création des séquences | Modifier chrono_seq_name par LOWER(chrono_seq_name)
        EXECUTE 'CREATE SEQUENCE "' || LOWER(chrono_seq_name) ||
                '" INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;'; -- EDISSYUM - NCH01 Fix de création des séquences | Modifier chrono_seq_name par LOWER(chrono_seq_name)
    END IF;
    ...
    SELECT nextval(LOWER(chrono_seq_name))
    INTO retval; -- EDISSYUM - NCH01 Fix de création des séquences | Modifier chrono_seq_name par LOWER(chrono_seq_name)
...
BEGIN
    ...
    FOR chrono IN (SELECT * FROM parameters WHERE id LIKE '%_' || extract(YEAR FROM current_date))
        LOOP
