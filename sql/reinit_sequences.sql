...
SELECT setval('difflist_roles_id_seq', (SELECT max(id)+1 FROM difflist_roles), false); -- EDISSYUM EME01 - MAJ le script de reinitialisation des sequences
SELECT setval('export_templates_id_seq', (SELECT max(id)+1 FROM export_templates), false); -- EDISSYUM - ASY01 Ajout de modèle d'exportation PDF/CSV