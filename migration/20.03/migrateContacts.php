...
foreach ($customs as $custom) {
    ...
    $contactsInfo = \SrcCore\models\DatabaseModel::select([
        'select' => ['contact_id', 'society', 'contact_firstname', 'contact_lastname', 'contact_title', 'contact_function', 'contact_other_data',
            ...
            'is_private'
        ], // EDISSYUM - NCH01 Amélioration de la migration des contacts confidentiels | Ajout de 'is_private'
        'table' => ['view_contacts']
    ]);
    ...
    $customFields= [
        ...
        ['oldId' => 'is_private',            'label' => 'Coordonnées confidentielles',         'type' => 'select',     'value' => ['Oui', 'Non']] // EDISSYUM - NCH01 Amélioration de la migration des contacts confidentiels
    ];

    $newCustomFields = addCustomFields(['customFields' => $customFields]);
    
    \Contact\models\ContactCustomFieldListModel::create([
        'label'  => 'Coordonnées confidentielles avancée',
        'type'   => 'select',
        'values' => json_encode(['Oui', 'Non'])
    ]);
    ...
    foreach ($contactsInfo as $contactInfo) {
        ...
        $contactCustomInfo = [
            ...
            'is_private'            => $contactInfo['is_private'], // EDISSYUM - NCH01 Amélioration de la migration des contacts confidentiels
        ];
        ...
        unset($contactInfo['is_private']); // EDISSYUM - NCH01 Amélioration de la migration des contacts confidentiels
...
function migrateCustomField($args = [])
{
    ...
    foreach ($args['contactCustomInfo'] as $key => $value) {
        if (!empty($value)) {
            ...
            // EDISSYUM - NCH01 Amélioration de la migration des contacts confidentiels
            if ($key == 'is_private') {
                $value = $value == '"Y"' ? '"Oui"' : '"Non"';
            }
            // END EDISSYUM - NCH01

            \Contact\models\ContactModel::update([
                'postSet' => ['custom_fields' => "jsonb_set(custom_fields, '{{$args['newCustomFields'][$key]}}', '{$value}')"], // EDISSYUM - NCH01 Amélioration de la migration des contacts confidentiels | Remplacer '{}' par custom_fields
                'where' => ['id = ?'],
