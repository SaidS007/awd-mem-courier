<?php
    ...
    foreach ($customs as $custom) {
        ...
        $actions = \Action\models\ActionModel::get(['select' => ['id', 'parameters', 'id_status'], 'where' => ['component in (?)'], 'data' => [$actionsToMigrate]]);
        foreach ($actions as $key => $action) {
            $parameters = json_decode($action['parameters'], true);
            $parameters['successStatus'] = $action['id_status'];
            $parameters = json_encode($parameters);
            \Action\models\ActionModel::update(['set' => ['parameters' => $parameters], 'where' => ['id = ?'], 'data' => [$action['id']]]);
            $migrated++;
        }
