        ...
        if ($loadedXml) {
            ...
            if (count($attachmentTypesXML) > 0) {
                foreach ($attachmentTypesXML->type as $value) {
                    ...
                    $attachmentTypesXml[(string)$value->id] = ['label' => $label, 'icon' => (string)$value['icon'], 'sign' => (empty($value['sign']) || (string)$value['sign'] == 'true') ? 'true' : 'false', 'chrono' => (empty($value['with_chrono']) || (string)$value['with_chrono'] == 'true') ? 'true' : 'false', 'attachInMail' => (!empty($value['attach_in_mail']) && (string)$value['attach_in_mail'] == 'true') ? 'true' : 'false', 'show' => (empty($value->attributes()->show) || (string)$value->attributes()->show == 'true') ? 'true' : 'false'];
                }
            }
        }
        if (!empty($attachmentTypesXml)) {
            foreach ($attachmentTypesXml as $key => $typeXml) {
                $type = ['type_id' => $key, 'label' => $typeXml['label'], 'visible' => $typeXml['show'], 'email_link' => $typeXml['attachInMail'], 'signable' => $typeXml['sign'], 'icon' => $typeXml['icon'], 'chrono' => $typeXml['chrono'], 'version_enabled' => 'true', 'new_version_default' => 'true', 'signed_by_default' => 'false'];
