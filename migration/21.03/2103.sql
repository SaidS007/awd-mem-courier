...
SELECT id, 'myLastResources', 'list', 1, '#A5DA7', '{}' FROM users WHERE status != 'DEL';

INSERT INTO tiles (user_id, type, view, position, color, parameters)
SELECT id, 'followedMail', 'chart', 0, '#A5DA7', '{"chartMode": "status", "chartType": "pie"}' FROM users WHERE status != 'DEL';
