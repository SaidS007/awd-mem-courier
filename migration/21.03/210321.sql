...
BEGIN
    ...
    IF NOT EXISTS (SELECT 0 FROM pg_class where relname = LOWER(chrono_seq_name)) THEN -- EDISSYUM - NCH01 Fix de création des séquences | Modifier chrono_seq_name par LOWER(chrono_seq_name)
        EXECUTE 'CREATE SEQUENCE "' || LOWER(chrono_seq_name) || '" INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;'; -- EDISSYUM - NCH01 Fix de création des séquences | Modifier chrono_seq_name par LOWER(chrono_seq_name)
    END IF;
    ...
    SELECT nextval(LOWER(chrono_seq_name)) INTO retval; -- EDISSYUM - NCH01 Fix de création des séquences | Modifier chrono_seq_name par LOWER(chrono_seq_name)
